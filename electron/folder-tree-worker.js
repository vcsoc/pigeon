...
const result = locations.map((location) => {
  ...
  const parentFolders=new Set([...folders.keys()].map((folderPath)=>folderPath.split('/').slice(0,-1).join('/')).filter(Boolean)),sorted = [...folders.values()].sort((a, b) => a.path.localeCompare(b.path, undefined, { numeric: true, sensitivity: 'base' })).map((folder) => ({ ...folder, hasChildren: parentFolders.has(folder.path) }));
