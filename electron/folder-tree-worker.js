...
const assetsByLocation = new Map(), locations = (workerData.locations || []).map((location) => ({ ...location, normalizedPath: String(location.path || '').replace(/\\/g, '/').replace(/\/$/, '') }));
...
const result = locations.map((location) => {
  ...
  if (!workerData.metadataOnly) discoverPhysicalFolders(location, folders, isExcluded);
  ...
  const parentFolders=new Set([...folders.keys()].map((folderPath)=>folderPath.split('/').slice(0,-1).join('/')).filter(Boolean)),sorted = [...folders.values()].sort((a, b) => folderCollator.compare(a.path,b.path)).map((folder) => ({ ...folder, hasChildren: parentFolders.has(folder.path) }));
  return { locationId: location.id, folders: sorted.slice(0, Math.max(100, workerData.limits?.[location.id] || 300)), visibleFolders: sorted.length, totalFolders: sorted.length };
});
