...
function similarImageGroups(assets, accuracy = 95, sourceId = null, options = {}) {
  const images = assets.filter((asset) => asset.kind === 'image' && !asset.deletedAt && !asset.locked), report = typeof options.onProgress === 'function' ? options.onProgress : () => {},checkpoint=typeof options.checkpoint==='function'?options.checkpoint:()=>{};
  if (sourceId) { const source = images.find((asset) => asset.id === sourceId); if (!source) return []; const matches=[];for(let index=0;index<images.length;index+=1){const asset=images[index];if(index%16===0)checkpoint();if(asset.id!==source.id&&visualSimilarityScore(source,asset)>=accuracy)matches.push(asset);if(index%256===0)report(index+1,images.length);}report(images.length,images.length);matches.sort((a,b)=>visualSimilarityScore(source,b)-visualSimilarityScore(source,a));return matches.length?[[source,...matches]]:[]; }
  ...
  for(let index=0;index<images.length;index+=1){if(index%16===0)checkpoint();const asset=images[index];if(asset.contentHash){if(exactHashes.has(asset.contentHash))join(index,exactHashes.get(asset.contentHash));else exactHashes.set(asset.contentHash,index);}if(/^[0-9a-f]{16}$/i.test(asset.perceptualHash||'')){searchHash(asset.perceptualHash,index,(candidate)=>{if(visualSimilarityScore(asset,images[candidate])>=accuracy)join(index,candidate);});insertHash(asset.perceptualHash,index);}if(index%128===0)report(index+1,workTotal);}
  for(let position=0;position<withoutHashes.length;position+=1){checkpoint();const index=withoutHashes[position];for(let candidate=0;candidate<images.length;candidate+=1){if(candidate%32===0)checkpoint();if(candidate!==index&&visualSimilarityScore(images[index],images[candidate])>=accuracy)join(index,candidate);}if(position%8===0)report(images.length+position+1,workTotal);}
