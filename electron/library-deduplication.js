...
function deduplicateAssetsByPath(library){
  ...
  // Siblings share folder ownership. Keep exact file roots and assets equal to
  // an indexed folder root separate from the directory cache.
  const folders=locations.filter((location)=>location.type!=='file'),fileOwners=new Map(locations.filter((location)=>location.type==='file').map((location)=>[normalizedPathKey(location.path),location])),folderRoots=new Set(folders.map((location)=>normalizedPathKey(location.path))),directoryOwners=new Map();
  const assets=[...withoutPath],removedIds=[];let changed=false;for(const [key,references] of groups){let owner=fileOwners.get(key);if(!owner){const candidate=normalizePath(references[0].path);if(folderRoots.has(key))owner=owningLocation(folders,candidate);else{const directory=pathApi(candidate).dirname(candidate);if(!directoryOwners.has(directory))directoryOwners.set(directory,owningLocation(folders,directory));owner=directoryOwners.get(directory);}}if(references.length===1&&(!owner||references[0].locationId===owner.id)){assets.push(references[0]);continue;}const ownerReferences=references.filter((asset)=>asset.locationId===owner?.id),primary=[...(ownerReferences.length?ownerReferences:references)].sort((a,b)=>assetRichness(b)-assetRichness(a))[0],merged=mergeAssetReferences(primary,references,owner);assets.push(merged);for(const asset of references)if(asset.id!==primary.id)removedIds.push(asset.id);changed=true;}
