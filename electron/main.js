...
ipcMain.handle('folder-tree:build', (_event, { collapsedKeys = [], limits = {} }) => new Promise((resolve) => { const locations = library.locations.map(({id,path})=>({id,path})), assets = library.assets.map(({locationId,path})=>({locationId,path})), emptyFolders=library.settings?.emptyFolders||{},worker = new Worker(path.join(__dirname, 'folder-tree-worker.js'), { workerData: { locations, assets, emptyFolders, collapsedKeys, limits } }), telemetry = trackWorker(worker, 'folder-tree', { filesTotal: assets.length }); let settled = false; const finish = (value) => { if (settled) return; settled = true; telemetry.filesCompleted = assets.length; worker.terminate().catch(() => {}); resolve(value || []); }; worker.once('message', finish); worker.once('error', (error) => { recordDiagnostic('error', 'Folder tree worker failed', error); finish([]); }); worker.once('exit', () => finish([])); }));
...
ipcMain.on('assets:start-drag',(event,ids=[])=>{const files=library.assets.filter((asset)=>ids.includes(asset.id)&&!asset.deletedAt&&!isAssetLocked(asset)&&fs.existsSync(asset.path)).map((asset)=>asset.path);if(!files.length)return;const icon=library.assets.find((asset)=>ids.includes(asset.id)&&asset.thumbnailPath&&fs.existsSync(asset.thumbnailPath))?.thumbnailPath||path.join(__dirname,'..','pigeon-logo.png');event.sender.startDrag({files,icon});});
