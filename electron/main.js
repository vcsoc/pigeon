...
async function warmThumbnailCache(){
  const scheduler=ensureThumbnailGenerationScheduler(),assets=library.assets,locations=new Map(library.locations.map((location)=>[location.id,location])),context={portfolioId:activePortfolioId,generation:libraryStreamGeneration},eligible=assets.filter((asset)=>!asset.sourcePending&&!asset.sourceMissing&&thumbnailWorkRequired(asset)&&locations.get(asset.locationId)?.online===true);let cursor=0;
