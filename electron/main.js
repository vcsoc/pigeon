...
async function saveImageEdits(id,edits={},annotations=null){
  ...
  const stamp=Date.now(),target=path.join(editDir,`${asset.id}-${stamp}.png`),previewTarget=editedPreviewTarget(asset,stamp),rotation=((Number(asset.rotation)||0)+Number(edits.rotate||0)+360)%360,result=await renderImageDerivative(source,target,{...edits,rotate:rotation,format:'png'});await createEditedPreview(target,previewTarget);
