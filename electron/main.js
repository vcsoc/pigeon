...
ipcMain.handle('tags:delete', (_event, requestedTags) => {
  const targets = new Set((Array.isArray(requestedTags) ? requestedTags : [requestedTags]).map((tag) => String(tag || '').trim().toLowerCase()).filter(Boolean)); let updatedAssets = 0;
  for (const asset of library.assets) { const tags = asset.tags || [], remaining = tags.filter((tag) => !targets.has(tag.toLowerCase())); if (remaining.length !== tags.length) { asset.tags = remaining; updatedAssets += 1; } }
  scheduleSave(); return { deletedTags: [...targets], updatedAssets };
});
...
ipcMain.on('assets:start-drag',(event,ids=[])=>{const selected=new Set(ids),assets=library.assets.filter((asset)=>selected.has(asset.id)&&!asset.deletedAt&&!isAssetLocked(asset)&&fs.existsSync(asset.path)),files=assets.map((asset)=>asset.path);if(!files.length)return;const iconPath=assets.find((asset)=>asset.thumbnailPath&&fs.existsSync(asset.thumbnailPath))?.thumbnailPath||path.join(__dirname,'..','pigeon-logo.png');let icon=nativeImage.createFromPath(iconPath);if(icon.isEmpty())icon=nativeImage.createFromPath(path.join(__dirname,'..','pigeon-logo.png'));if(!icon.isEmpty()){icon=icon.resize({width:96,height:96,quality:'good'});const size=icon.getSize(),bitmap=icon.toBitmap();for(let index=3;index<bitmap.length;index+=4)bitmap[index]=Math.round(bitmap[index]*.34);icon=nativeImage.createFromBitmap(bitmap,{width:size.width,height:size.height,scaleFactor:1});}try{event.sender.startDrag({files,icon});}catch(error){recordDiagnostic('error','Native asset drag failed',{message:error.message,count:files.length});}});
