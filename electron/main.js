...
const THUMBNAIL_REBUILD_WORKER_COUNT = Math.max(2,Math.min(4,Math.floor((os.availableParallelism?.()||os.cpus().length||2)/2)));
...
const scanPreviewSessions=require('./scan-preview-sessions').createScanPreviewSessions({
  begin:location=>beginBackgroundRun('scan-previews',location.id),isActive:backgroundRunActive,finish:finishBackgroundRun,maxConcurrency:THUMBNAIL_WORKER_COUNT,
