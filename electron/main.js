...
function editedPreviewTarget(asset,stamp=asset.editedAt||Date.now()){return path.join(thumbnailDir,'edits',`${asset.id}-${stamp}-preview.jpg`);}
...
async function enlargeImageAsset(id,scale=2){
  const asset=library.assets.find((item)=>item.id===id);if(!asset||isAssetLocked(asset)||!isEditableVisualAsset(asset))throw new Error('Select an unlocked image to enlarge');
  const source=await editableAssetSource(asset),editDir=path.join(thumbnailDir,'edits');await fsp.mkdir(editDir,{recursive:true});const stamp=Date.now(),target=path.join(editDir,`${asset.id}-${stamp}-ai.png`),previewTarget=editedPreviewTarget(asset,stamp);let result;
  try{result=await enlargeImageWithAi(source,target,{scale,rotation:Number(asset.rotation)||0,previewTarget});}catch(error){await Promise.all([fsp.rm(target,{force:true}),fsp.rm(previewTarget,{force:true})]);throw error;}
  await Promise.all([asset.editedPath&&asset.editedPath!==target?fsp.rm(asset.editedPath,{force:true}):null,asset.editedPreviewPath&&asset.editedPreviewPath!==previewTarget?fsp.rm(asset.editedPreviewPath,{force:true}):null].filter(Boolean));
  ...
  scheduleAssetSave(asset);const update=publicEditedAsset(asset);broadcastAssetPatches([{id:asset.id,editedPath:target,editedPreviewPath:previewTarget,editedAt:asset.editedAt,inlineCrop:null,imageAdjustments:null,imageResize:asset.imageResize,annotations:[],width:asset.width,height:asset.height,rotation:0,previewUrl:update.previewUrl,mediaUrl:update.mediaUrl}]);return update;
}
...
ipcMain.handle('asset:ai-remove-preview',(_event,{id,maskDataUrl,previousToken})=>runAiRemoval(id,maskDataUrl,previousToken));
