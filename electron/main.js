...
async function registerProtocol() {
  protocol.handle('pigeon-asset', async (request) => {
    ...
    const candidates=wantsOriginal?[wantsEdited&&asset.editedPath?asset.editedPath:wantsProxy && asset.proxyPath?asset.proxyPath:asset.path]:wantsEdited&&asset.editedPath?(editedPreviewReady?[asset.editedPreviewPath]:asset.thumbnailPath?[asset.thumbnailPath]:[]):asset.thumbnailPath?[asset.thumbnailPath]:asset.kind!=='image'?[asset.path]:[];
...
async function saveImageEdits(id,edits={},annotations=null){
  ...
  const stamp=Date.now(),target=path.join(editDir,`${asset.id}-${stamp}.png`),previewTarget=editedPreviewTarget(asset,stamp),rotation=((Number(asset.rotation)||0)+Number(edits.rotate||0)+360)%360,result=await renderImageDerivative(source,target,{...edits,rotate:rotation,format:'png',annotations:Array.isArray(annotations)?annotations.filter(require('../src/editor-region-effects').isEffect):[]});await createEditedPreview(target,previewTarget);
  await Promise.all([asset.editedPath&&asset.editedPath!==target?fsp.rm(asset.editedPath,{force:true}):null,asset.editedPreviewPath&&asset.editedPreviewPath!==previewTarget?fsp.rm(asset.editedPreviewPath,{force:true}):null].filter(Boolean));
  Object.assign(asset,{editedPath:target,editedPreviewPath:previewTarget,editedAt:stamp,inlineCrop:edits.crop||null,imageAdjustments:{grayscale:Boolean(edits.grayscale),negative:Boolean(edits.negative),sepia:Boolean(edits.sepia),brightness:Number(edits.brightness)||1,contrast:Number(edits.contrast)||1},imageResize:edits.resize||null,annotations:Array.isArray(annotations)?annotations.filter(item=>!require('../src/editor-region-effects').isEffect(item)):asset.annotations||[],width:result.width,height:result.height,rotation:0});
  scheduleAssetSave(asset);const update=publicEditedAsset(asset);broadcastAssetPatches([{id:asset.id,editedPath:asset.editedPath,editedPreviewPath:asset.editedPreviewPath,editedAt:asset.editedAt,inlineCrop:asset.inlineCrop,imageAdjustments:asset.imageAdjustments,imageResize:asset.imageResize,annotations:asset.annotations,width:asset.width,height:asset.height,rotation:0,previewUrl:update.previewUrl,mediaUrl:update.mediaUrl}]);return update;
}
...
async function resetInlineEdits(id){
  const asset=library.assets.find((item)=>item.id===id);if(!asset)throw new Error('Asset not found');await Promise.all([asset.editedPath?fsp.rm(asset.editedPath,{force:true}):null,asset.editedPreviewPath?fsp.rm(asset.editedPreviewPath,{force:true}):null].filter(Boolean));
  asset.editedPath=null;asset.editedPreviewPath=null;asset.editedAt=null;asset.inlineCrop=null;asset.imageAdjustments=null;asset.imageResize=null;asset.rotation=0;
...
async function enlargeImageAsset(id,scale=2){
  ...
  const previousPaths=[asset.editedPath&&asset.editedPath!==target?asset.editedPath:null,asset.editedPreviewPath&&asset.editedPreviewPath!==previewTarget?asset.editedPreviewPath:null].filter(Boolean);
  Object.assign(asset,{editedPath:target,editedPreviewPath:previewTarget,editedAt:stamp,inlineCrop:null,imageAdjustments:null,imageResize:{width:result.width,height:result.height,scale:result.scale,algorithm:'ai-super-resolution'},annotations:[],width:result.width,height:result.height,rotation:0});
