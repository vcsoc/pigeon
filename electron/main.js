...
const EMBEDDED_METADATA_VERSION=1;
function thumbnailWorkRequired(asset) {
  if(asset.extension==='PNG'&&asset.embeddedMetadataVersion!==EMBEDDED_METADATA_VERSION)return true;
...
async function warmThumbnailCache() {
  ...
  const workers = Array.from({ length: Math.min(THUMBNAIL_WORKER_COUNT, pending.length) }, async () => {
    while (pending.length && backgroundRunActive(run)) {
      ...
      if(metadataOnly){asset.embeddedMetadata=thumbnail.embeddedMetadata||null;asset.embeddedMetadataVersion=EMBEDDED_METADATA_VERSION;completedSinceSave+=1;changedPreviewAssets.push(asset);if(mainWindow&&!mainWindow.isDestroyed())mainWindow.webContents.send('thumbnail:ready',{id:asset.id,embeddedMetadata:asset.embeddedMetadata});if(completedSinceSave>=100){completedSinceSave=0;await persistAssetBatch(changedPreviewAssets.splice(0));}continue;}
      asset.thumbnailFailedAt = null; asset.thumbnailFailedModified = null; asset.thumbnailError = null;asset.thumbnailFailureVersion=null; asset.thumbnailPath = thumbnail.target;if(asset.extension==='PDF')asset.pdfPreviewVersion=PDF_PREVIEW_VERSION;if(asset.extension==='PNG')asset.embeddedMetadataVersion=EMBEDDED_METADATA_VERSION;
...
ipcMain.handle('asset:embedded-metadata',async(_event,id)=>{const asset=library.assets.find((item)=>item.id===id);if(!asset||asset.extension!=='PNG'||isAssetLocked(asset))return{embeddedMetadata:null,embeddedMetadataVersion:EMBEDDED_METADATA_VERSION};if(asset.embeddedMetadataVersion!==EMBEDDED_METADATA_VERSION){const result=await extractEmbeddedMetadata(asset);asset.embeddedMetadata=result?.embeddedMetadata||null;asset.embeddedMetadataVersion=EMBEDDED_METADATA_VERSION;await persistAssetBatch([asset]);}return{embeddedMetadata:asset.embeddedMetadata||null,embeddedMetadataVersion:asset.embeddedMetadataVersion};});
