...
3. **Media previews and file handoff** — the inspector plays local video and audio through a seekable custom protocol with byte-range responses. Affinity `.af`, `.afdesign`, and `.afphoto` documents and Snagit `.snagx` archives expose their rendered embedded images safely for grid thumbnails, the inspector, and the internal viewer. Drag thumbnails directly into Finder, Explorer, Slack, Obsidian, Outlook, Word, Excel, PowerPoint, or any application that accepts operating-system file drops; hold Shift while dragging to organize assets within Pigeon.
