...
2. **Multi-select and batch editing** — thumbnail selection paints on pointer-down while preserving an existing multi-selection for drag gestures. Ctrl/Cmd-click, Shift-click, keyboard navigation, and the batch bar support tags, collections, automatic tags, favorites, and reference trash.
