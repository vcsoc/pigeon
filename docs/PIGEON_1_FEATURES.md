...
3. **Media previews** — the inspector plays local video and audio through a seekable custom protocol with byte-range responses. Snagit `.snagx` archives are indexed as documents, with their rendered embedded image extracted safely for grid thumbnails, the inspector, and the internal viewer.
