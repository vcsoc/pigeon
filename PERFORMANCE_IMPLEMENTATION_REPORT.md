...
## Responsive virtual-layout density follow-up

The rating/virtual-scroll build was manually rejected because generated thumbnails occupied a sparse surface with large gaps. The owner was a split layout model: the result-scoped scroll extent used renderer-estimated fixed columns and rows, while CSS independently reflowed cards for the available content width. Resizing the window, inspector, or sidebar did not invalidate the renderer metrics. Virtual justified mode also forced every card to one fixed width, disabling the aspect-ratio packing used by the nonvirtual layout.

The virtual renderer now uses one deterministic layout model for both scroll extent and card coordinates. Grid mode packs mixed aspect ratios into the shortest available column with only the configured four-pixel gap; justified mode calculates dense equal-height aspect rows and fills every complete row; list mode uses one full-width card per logical row. Every visible range is a contiguous result-index slice. Card positions, surface extent, window selection, diagnostics, and responsive column/row calculations all consume the same model.

Viewport, panel-width, thumbnail-size, layout-mode, rotation, and late dimension changes create a new cached layout generation. The current logical asset anchor is mapped once into the new generation, existing keyed card/image nodes are reused, and user input cancels pending programmatic placement. Extent is always replaced by the newly calculated exact result extent; it is never ratcheted from DOM or decode measurements. Unchanged scroll frames reuse the cached full-result geometry rather than rebuilding it.

The deterministic density gate covers 5,198 mixed-ratio results at representative viewport/sidebar/inspector widths and thumbnail sizes across grid, justified, and list modes. It asserts contiguous indices, no empty logical slots, only configured inter-card/row spacing, complete justified-row fill, exact extent, responsive anchor preservation, late-ratio rebuilding, and bottom→up→down addressability. Syntax checks pass; the focused layout/performance/Smart Folder gate passes 154/154; the complete suite passes 230/230 in 1.78 seconds. The representative 41,293-asset projection used 83 yielded batches and reported a 31.0 ms largest synchronous slice on this run.
