(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.PigeonAssetStreamState=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  function create({minimumViewport=120,onReset=()=>{},onUpsert=()=>{},onPatch=()=>{},onRemove=()=>{}}={}){
    ...
    const removeMany=(ids=[])=>{const removed=new Set(ids),previous=library.assets;if(!removed.size)return{removed:0};library.assets=previous.filter((asset)=>!removed.has(asset.id));for(const asset of previous)if(removed.has(asset.id))onRemove(asset);indexes=new Map(library.assets.map((asset,index)=>[asset.id,index]));library.totalAssets=library.assets.length;return{removed:previous.length-library.assets.length};};
    return{begin,applyChunk,finish,upsertMany,patch,removeMany,get library(){return library;},get generation(){return generation;},get indexes(){return indexes;},get complete(){return complete;},get firstUsable(){return firstUsable;}};
  }
  return{create};
});
