...
async function refreshAiRemovalEditorStatus(){const status=$('#ai-removal-editor-status'),run=$('#run-ai-removal');if(!status||!run)return;try{const plugin=(await window.pigeon.listPlugins()).find((item)=>item.id==='ai-removal');const ready=Boolean(plugin?.installed&&plugin.modelReady&&plugin.enabled);run.disabled=!ready;status.dataset.status=ready?'ready':plugin?.status==='error'?'error':'setup';status.textContent=ready?(plugin.status==='running'?'Simple LaMa is ready on this computer.':'Simple LaMa is enabled and will start automatically.'):!plugin?.installed?'Install and set up Simple LaMa in Plugin Manager.':!plugin.modelReady?(plugin.modelState?.partialBytes?`Model download can be resumed from ${formatPluginBytes(plugin.modelState.partialBytes)}.`:'Download and validate the Simple LaMa model.'):'Enable AI Object Removal in Plugin Manager.';}catch(error){run.disabled=true;status.dataset.status='error';status.textContent=`Plugin status unavailable · ${error.message}`;}}
...
async function requestAiRemovalPreview(){if(!state.aiMaskPainted){showToast('Paint over the object you want removed');return;}const button=$('#run-ai-removal');button.disabled=true;button.textContent='Removing…';try{const result=await window.pigeon.previewAiRemoval(state.selectedId,$('#ai-removal-mask').toDataURL('image/png'),state.aiRemovalToken);state.aiRemovalToken=result.token;$('#ai-removal-preview').src=result.previewUrl;$('#ai-removal-review').classList.remove('hidden');}catch(error){showToast(`${error.message} · Open Plugin Manager to repair AI Object Removal`);}finally{button.textContent='Remove object';await refreshAiRemovalEditorStatus();}}
