...
function attachHoverMediaPreview(card, asset) {
  ...
  const start=(event)=>{if(!enabled()||media)return;controlHeld=Boolean(event?.ctrlKey);if(animatedImage){media=document.createElement('img');media.className='thumbnail-hover-media animated-image';media.alt='';media.src=protectedUrl(asset.mediaUrl);preview.appendChild(media);preview.classList.add('animated-hovering');return;}media=document.createElement(asset.kind);media.className=`thumbnail-hover-media ${asset.kind}`;media.src=asset.mediaUrl;media.preload='metadata';media.loop=true;media.muted=asset.kind==='video';media.playsInline=true;preview.appendChild(media);if(asset.kind==='video'){syncHoverVideoSound();hoveredVideoSoundSync=syncHoverVideoSound;window.pigeon.monitorHoverControl(true);window.addEventListener('keydown',keyDown,true);window.addEventListener('keyup',keyUp,true);}preview.classList.add('media-hovering');media.addEventListener('loadedmetadata',()=>{if(desiredTime!==null)media.currentTime=desiredTime;media.play().catch(()=>{});},{once:true});media.play().catch(()=>{});};
...
function showCollectionContextMenu(event, collection) {
  ...
  elements.contextMenu.querySelectorAll('[data-folder-action]').forEach((button) => button.addEventListener('click', async () => {
    const action = button.dataset.folderAction;if(action==='order'){showBranchOrderMenu(button,{type:'collections',branch:collection.id,label:collection.name});return;} hideContextMenu();
    try {
...
function showLocationContextMenu(event, location, subfolder = '', row = null) {
  ...
  elements.contextMenu.querySelectorAll('[data-location-action]').forEach((button) => button.addEventListener('click', async () => {
    const action = button.dataset.locationAction;if(action==='order'){showBranchOrderMenu(button,{type:'folders',branch:`${location.id}:${subfolder}`,label:subfolder?subfolder.split('/').pop():location.name});return;} hideContextMenu();
...
function showBranchOrderMenu(anchor,{type,branch,label}){const current=sidebarSortValue(type,branch);elements.contextMenu.innerHTML=`<div class="context-menu-title">Order ${escapeHtml(label)} by</div>${branchSortOptions.map(([value,text])=>`<button data-branch-sort="${value}"><span>${text}</span><span class="menu-check">${current===value?'✓':''}</span></button>`).join('')}`;elements.contextMenu.querySelectorAll('[data-branch-sort]').forEach((button)=>button.addEventListener('click',async()=>{hideContextMenu();state.library.settings=state.library.settings||{};state.library.settings.sidebarBranchSort={...(state.library.settings.sidebarBranchSort||{}),[`${type}:${branch??'root'}`]:button.dataset.branchSort};renderSidebar(false);await window.pigeon.setSidebarBranchSort(type,branch,button.dataset.branchSort);}));elements.contextMenu.classList.remove('hidden');positionMenu(elements.contextMenu,anchor);}
function renderSidebar(rebuildFolderTree = false) {
  ...
  $$('#smart-folder-list [data-smart-folder-id]').forEach((button) => {
    ...
    button.addEventListener('contextmenu', (event) => {
      ...
      elements.contextMenu.querySelectorAll('[data-smart-action]').forEach((actionButton) => actionButton.addEventListener('click', async () => { const action = actionButton.dataset.smartAction;if(action==='order'){showBranchOrderMenu(actionButton,{type:'smartFolders',branch:folder.id,label:folder.name});return;} hideContextMenu();
...
function hideContextMenu() { elements.contextMenu.classList.add('hidden');elements.contextMenu.classList.remove('asset-context-menu-expanded'); }
