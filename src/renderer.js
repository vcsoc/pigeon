...
function closeAnnotationEditor(){if(state.aiRemovalToken)window.pigeon.discardAiRemoval(state.aiRemovalToken).catch(()=>{});state.aiRemovalToken=null;elements.annotationView.classList.add('hidden');renderGrid();renderInspector();}
...
async function requestAiRemovalPreview(){
  ...
  const status=$('#ai-removal-editor-status');
  ...
  try{
    ...
    const result=await window.pigeon.previewAiRemoval(state.selectedId,$('#ai-removal-mask').toDataURL('image/png'),state.aiRemovalToken);state.aiRemovalToken=result.token;$('#ai-removal-preview').src=result.previewUrl;$('#ai-removal-review').classList.remove('hidden');completed=true;showToast('AI removal preview is ready · Accept, Retry, or Discard');
  }catch(error){status.dataset.status='error';status.textContent=`AI removal failed · ${error.message}`;showToast(`${error.message} · Open Plugin Manager to repair AI Object Removal`);}
  finally{setAiRemovalProgress(false);if(completed)await refreshAiRemovalEditorStatus();}
}
$('#run-ai-removal').addEventListener('click',requestAiRemovalPreview);$('#retry-ai-removal').addEventListener('click',()=>{$('#ai-removal-review').classList.add('hidden');requestAiRemovalPreview();});$('#discard-ai-removal').addEventListener('click',async()=>{if(state.aiRemovalToken)await window.pigeon.discardAiRemoval(state.aiRemovalToken);state.aiRemovalToken=null;$('#ai-removal-review').classList.add('hidden');});$('#accept-ai-removal').addEventListener('click',async()=>{if(!state.aiRemovalToken)return;try{const updated=await window.pigeon.acceptAiRemoval(state.selectedId,state.aiRemovalToken),asset=assetById(state.selectedId);state.aiRemovalToken=null;if(asset&&updated){Object.assign(asset,updated);rendererAssetDerivatives.upsert(asset);elements.annotationStage.style.backgroundImage=`url("${protectedUrl(asset.mediaUrl)}")`;}$('#ai-removal-review').classList.add('hidden');reconcileThumbnailCards([state.selectedId],{sidebar:false});showToast('AI removal accepted non-destructively');}catch(error){showToast(error.message);}});
...
$('#save-annotations').addEventListener('click',async()=>{try{const updated=await window.pigeon.saveImageEdits(state.selectedId,state.workingEdits,state.workingAnnotations),asset=assetById(state.selectedId);if(asset&&updated){Object.assign(asset,updated);rendererAssetDerivatives.upsert(asset);}closeAnnotationEditor();reconcileThumbnailCards([state.selectedId],{sidebar:false});showToast('Image edits saved non-destructively');}catch(error){showToast(error.message);}});
$('#export-annotations').addEventListener('click',async()=>{try{const target=await window.pigeon.exportAnnotated(state.selectedId,state.workingAnnotations,state.workingEdits);if(target)showToast(`Saved copy ${target}`);}catch(error){showToast(error.message);}});
