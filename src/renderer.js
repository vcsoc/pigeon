...
function temporaryShortcutPressed(event,configured){const parts=String(configured||'').replace('Control','Ctrl').split('+').filter(Boolean),key=parts.at(-1),eventKey=event.key==='Control'?'Ctrl':event.key==='Meta'?'Meta':event.key.length===1?event.key.toUpperCase():event.key,required=new Set(parts.slice(0,-1));return eventKey===key&&(!required.has('Ctrl')||event.ctrlKey)&&(!required.has('Alt')||event.altKey)&&(!required.has('Shift')||event.shiftKey)&&(!required.has('Meta')||event.metaKey);}
