const state = {
  ...
  locationSelectionAnchorId: null
};
...
function showLocationContextMenu(event, location, subfolder = '', row = null) {
  ...
  elements.contextMenu.querySelectorAll('[data-location-action]').forEach((button) => button.addEventListener('click', async () => {
    ...
    if(action==='delete'){if(!(await ensureFolderUnlocked(location,subfolder)))return;const name=subfolder?subfolder.split('/').pop():location.name,confirmed=await requestConfirmation({title:`Delete “${name}”?`,message:`Delete the physical folder “${name}” from the drive?\n\nEmpty folders are removed immediately. Non-empty folders are moved to the operating-system Recycle Bin.`,confirmText:'Delete Folder'});if(!confirmed)return;const result=await window.pigeon.deletePhysicalFolder(location.id,subfolder);if(state.locationId===location.id&&(!subfolder||state.locationSubfolder===subfolder||state.locationSubfolder.startsWith(`${subfolder}/`))){state.locationId=null;state.locationSubfolder='';state.view='all';}showToast(result.empty?'Empty folder deleted':'Folder moved to the Recycle Bin');return;}
...
function renderSidebar(rebuildFolderTree = false) {
  ...
  if(locationsRebuilt) $$('.location-item').forEach((row) => {
    row.querySelector('.location-root-button').addEventListener('click', (event) => { selectLocationRoots(event,row.dataset.locationId);selectLocation(row.dataset.locationId);renderSidebar(false); });
