...
$('.viewer-stage').addEventListener('wheel',(event)=>{if(elements.viewerImage.classList.contains('hidden'))return;event.preventDefault();const stage=$('.viewer-stage'),rect=stage.getBoundingClientRect(),cursorX=event.clientX-rect.left,cursorY=event.clientY-rect.top,oldZoom=state.viewerFit?Math.min(stage.clientWidth/Math.max(elements.viewerImage.naturalWidth,1),stage.clientHeight/Math.max(elements.viewerImage.naturalHeight,1)):state.viewerZoom,contentX=stage.scrollLeft+cursorX,contentY=stage.scrollTop+cursorY,factor=Math.exp(-Math.max(-80,Math.min(80,event.deltaY))*.0018),next=Math.max(.03,Math.min(12,oldZoom*factor)),scale=next/Math.max(oldZoom,.001);state.viewerFit=false;state.viewerZoom=next;elements.mediaViewer.classList.add('full-view');elements.viewerMinimap.classList.remove('hidden');$('#viewer-fit').textContent='Full';applyViewerImageFit();requestAnimationFrame(()=>{stage.scrollLeft=contentX*scale-cursorX;stage.scrollTop=contentY*scale-cursorY;updateViewerMinimap();});},{passive:false});
$('#contact-sheet-columns').addEventListener('input',renderContactSheet);$('#contact-sheet-rows').addEventListener('input',renderContactSheet);$('#contact-sheet-order').addEventListener('change',renderContactSheet);$('#contact-sheet-labels').addEventListener('click',(event)=>{state.contactSheetLabels=!state.contactSheetLabels;event.currentTarget.setAttribute('aria-pressed',String(state.contactSheetLabels));renderContactSheet();});$('#contact-sheet-print').addEventListener('click',()=>window.print());$('#contact-sheet-close').addEventListener('click',closeContactSheet);$('.sidebar').addEventListener('click',closeContactSheet);
