...
function showLocationContextMenu(event, location, subfolder = '', row = null) {
  ...
  elements.contextMenu.querySelectorAll('[data-location-action]').forEach((button) => button.addEventListener('click', async () => {
    ...
    if(action==='new-subfolder'){const name=await requestText({title:'New Subfolder',message:`Create a physical folder inside “${subfolder?subfolder.split('/').pop():location.name}”.`,label:'Folder name',placeholder:'New folder',confirmText:'Create'});if(name===null)return;try{const created=await window.pigeon.createPhysicalSubfolder(location.id,subfolder,name);setFolderCollapsed(collapseKey,false);scheduleFolderTreeBuild();showToast(`Created ${created.name}`);}catch(error){showToast(error.message);}return;}
...
async function createCollectionPrompt(parentId = null) {
  ...
  state.library.collections.push(optimistic);renderSidebar(false);
