...
function showAssetContextMenu(event, id) {
  ...
  const selectedImageIds=[...state.selectedIds].filter((assetId)=>assetById(assetId)?.kind==='image'),rotationTargetLabel = selectedImageIds.length > 1 ? `Rotate ${selectedImageIds.length} images` : 'Rotate';
...
$('#save-annotations').addEventListener('click',async()=>{const updated=await window.pigeon.updateAsset(state.selectedId,{annotations:state.workingAnnotations}),asset=assetById(state.selectedId);if(asset&&updated)Object.assign(asset,updated);closeAnnotationEditor();showToast('Annotations saved');});
