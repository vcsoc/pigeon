...
function gridViewIdentity(){return JSON.stringify([state.library.activePortfolioId||'',state.streamGeneration,state.view,state.locationId,state.locationSubfolder,state.collectionId,state.smartFolderId,state.kind,state.layout,state.query,[...state.filters.extensions],[...state.filters.ratings],[...state.filters.shapes],[...state.filters.locations],[...state.filters.tags],[...state.filters.colors]]);}
...
function renderSidebar(rebuildFolderTree = false) {
  ...
  scheduleLibraryAggregateBuild();const collapsedKeys=collapsedFolders(),assets=state.library.assets,metrics=libraryAggregateCache?.revision===libraryAggregateRevision?libraryAggregateCache:null,collectionCounts=metrics?.collectionCounts||new Map();
  ...
  if(locationsRebuilt) $$('.location-item').forEach((row) => {
    ...
    const enablePhysicalFolderDrop = (button, subfolder = '') => {
      const showPhysicalFolderDrop=(event)=>{const folderMove=event.dataTransfer.types.includes('application/x-pigeon-physical-folder'),internal=event.dataTransfer.types.includes('application/x-pigeon-assets');if(!folderMove&&!internal&&!hasExternalFiles(event))return;event.preventDefault();event.stopPropagation();event.dataTransfer.dropEffect=folderMove||internal?'move':'copy';button.classList.add('drag-over');};button.addEventListener('dragenter',showPhysicalFolderDrop);button.addEventListener('dragover',showPhysicalFolderDrop);
      ...
      button.addEventListener('drop',async(event)=>{
        const folderMove=event.dataTransfer.types.includes('application/x-pigeon-physical-folder'),internal=event.dataTransfer.types.includes('application/x-pigeon-assets');if(!folderMove&&!internal&&!hasExternalFiles(event))return;event.preventDefault();event.stopPropagation();button.classList.remove('drag-over');
        try{
...
async function selectLocation(id, subfolder = '') {
  ...
  hideInternalViewer();
  state.view='all';state.locationId = id; state.locationSubfolder = subfolder; state.collectionId = null; state.smartFolderId = null; state.selectedId = null;if(state.kind==='visual')state.kind='all'; state.gridScrollTop = 0; clearSelection(); resetRenderLimit();updateFilterChips();
...
async function selectCollection(id) {
  ...
  hideInternalViewer();
  state.view='all';state.collectionId = id; state.locationId = null;state.locationSubfolder=''; state.smartFolderId = null; state.selectedId = null;if(state.kind==='visual')state.kind='all'; state.gridScrollTop = 0; clearSelection(); resetRenderLimit();updateFilterChips();
...
function renderInternalViewer() {
  ...
  requestAnimationFrame(() => { applyViewerImageFit(); if (state.viewerFit) { $('.viewer-stage').scrollTo(0, 0); } updateViewerMinimap(); updateViewerCropOverlay();renderPixelatedSurface($('#viewer-image-surface'),elements.viewerImage,Boolean(asset.thumbnailEffect&&image)); });
}
...
async function createCollectionPrompt(parentId = null) {
  const parent = state.library.collections.find((item) => item.id === parentId), name = await requestText({ title: parent ? `New Nested Collection in ${parent.name}` : 'Create a Collection', message: parent ? 'This virtual collection will appear beneath its parent without changing any folders on disk.' : 'Collections organize references without moving or changing the original files.', label: 'Collection name', placeholder: 'Collection name', confirmText: parent ? 'Create Collection' : 'Create Collection' });
