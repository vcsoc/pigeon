...
elements.grid.addEventListener('dragstart',(event)=>{const card=event.target.closest('.asset-card');if(!card)return;if(marqueeSelection){event.preventDefault();return;}const ids=state.selectedIds.has(card.dataset.assetId)?[...state.selectedIds]:[card.dataset.assetId];event.dataTransfer.setData('application/x-pigeon-assets',JSON.stringify(ids));event.dataTransfer.setData('application/x-pigeon-origin',JSON.stringify({collectionId:state.collectionId||null,locationId:state.locationId||null,subfolder:state.locationSubfolder||''}));const draggedAssets=ids.map((id)=>{const index=rendererAssetIndexes.get(id);return index===undefined?null:state.library.assets[index];}).filter(Boolean),paths=draggedAssets.map((asset)=>asset.path).filter(Boolean);event.dataTransfer.effectAllowed = 'move';event.dataTransfer.effectAllowed='copyMove';event.dataTransfer.setData('text/plain',paths.join('\n'));event.dataTransfer.setData('text/uri-list',paths.map((filePath)=>`file:///${filePath.replace(/\\/g,'/').replace(/^\/+/, '')}`).join('\r\n'));if(draggedAssets[0]?.mediaUrl)event.dataTransfer.setData('DownloadURL',`application/octet-stream:${draggedAssets[0].filename}:${draggedAssets[0].mediaUrl}`);card._pigeonDragIds=ids;card._pigeonNativeDragStarted=false;});
function startNativeAssetDrag(card){if(!card||card._pigeonNativeDragStarted||!card._pigeonDragIds?.length)return;card._pigeonNativeDragStarted=true;window.pigeon.startAssetDrag(card._pigeonDragIds);}
elements.grid.addEventListener('drag',(event)=>{const card=event.target.closest?.('.asset-card');if(!card)return;const outside=(event.clientX<=1&&event.clientY<=1)||event.clientX<0||event.clientY<0||event.clientX>=window.innerWidth-1||event.clientY>=window.innerHeight-1;if(outside)startNativeAssetDrag(card);});elements.grid.addEventListener('dragleave',(event)=>{if(!event.relatedTarget)startNativeAssetDrag(event.target.closest?.('.asset-card'));});elements.grid.addEventListener('dragend',(event)=>{const card=event.target.closest?.('.asset-card');if(card){delete card._pigeonDragIds;delete card._pigeonNativeDragStarted;}});
