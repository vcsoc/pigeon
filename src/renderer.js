...
let deferredHierarchyRenderFrame=null;
function paintFolderCollapseImmediately(key,collapsed){const toggle=document.querySelector(`[data-collapse-key="${CSS.escape(key)}"]`),row=toggle?.closest('.collection-item,.smart-folder-item,.location-folder-item,.location-root-button');if(!toggle||!row)return;toggle.setAttribute('aria-expanded',String(!collapsed));toggle.textContent=collapsed?'▸':'▾';if(row.classList.contains('location-root-button')){const list=row.closest('.location-item')?.querySelector('.location-subfolder-list');if(list)list.hidden=collapsed;return;}const depth=Number.parseInt(row.style.getPropertyValue('--depth'),10)||0;for(let sibling=row.nextElementSibling;sibling;sibling=sibling.nextElementSibling){if(!sibling.matches('.collection-item,.smart-folder-item,.location-folder-item'))continue;const siblingDepth=Number.parseInt(sibling.style.getPropertyValue('--depth'),10)||0;if(siblingDepth<=depth)break;sibling.hidden=collapsed;}}
function toggleFolderCollapsed(key) { const collapsed=!isFolderCollapsed(key);setFolderCollapsed(key,collapsed);paintFolderCollapseImmediately(key,collapsed);if(key.startsWith('location:')||key.startsWith('subfolder:')){scheduleFolderTreeBuild();return;}if(deferredHierarchyRenderFrame!==null)cancelAnimationFrame(deferredHierarchyRenderFrame);deferredHierarchyRenderFrame=requestAnimationFrame(()=>requestAnimationFrame(()=>{deferredHierarchyRenderFrame=null;renderSidebar(false);})); }
...
function isDownloadProgressTask(task){const id=String(task?.id||''),label=String(task?.label||'');return id.startsWith('extension-import:')||id.includes(':url-import:')||id.startsWith('application:update:')||/^download(?:ed|ing)?\b/i.test(label);}
...
window.pigeon.onBackgroundProgress((task) => {
  ...
  const previous=backgroundTasks.get(task.id),terminal=Boolean(task.done)||['completed','failed','warning'].includes(task.status)||(!isDownloadProgressTask(task)&&task.total>0&&task.completed>=task.total),extensionImport=String(task.id||'').startsWith('extension-import:');backgroundTasks.set(task.id,{...task,done:terminal});renderThreadsPanel();
...
window.pigeon.listBackgroundThreads().then((tasks)=>{for(const task of tasks||[])if(!task.done)backgroundTasks.set(task.id,task);renderThreadsPanel();}).catch(()=>{});
...
window.pigeon.onLibraryChanged((library) => {
  ...
  state.library = assetStreamState.begin(library); invalidateTagCache(); state.duplicateGroups = []; state.duplicateIds.clear();
