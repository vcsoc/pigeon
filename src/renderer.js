...
function matchesSmartRule(asset, rule) {
  const values = rule.field === 'tags' ? asset.tags || [] : [rule.field === 'name' ? asset.filename || asset.name : rule.field === 'type' ? asset.kind : rule.field === 'folder' ? asset.path : rule.field === 'rating' ? String(asset.rating || 0) : rule.field === 'favorite' ? String(Boolean(asset.favorite)) : ''];
  const expected = String(rule.value || '').toLowerCase(), operator = rule.operator || 'contains';
  if (operator === 'null') return values.every((value) => !String(value).trim()); if (operator === 'not-null') return values.some((value) => String(value).trim());
...
function renderSidebar() {
  ...
  appendSmartFolders(); $('#smart-folder-list').innerHTML = smartRows.join('') || '<div class="facet-empty">No smart folders</div>';
  ...
  elements.locationList.innerHTML = state.library.locations.map((location) => {
    const root = String(location.path || '').replace(/\\/g, '/').replace(/\/$/, '');
    const subfolders = new Map();
    for (const asset of assets.filter((item) => item.locationId === location.id)) {
      const fullPath = String(asset.path || '').replace(/\\/g, '/');
      if (!fullPath.toLowerCase().startsWith(`${root}/`.toLowerCase())) continue;
      const directoryParts = fullPath.slice(root.length + 1).split('/').slice(0, -1);
      for (let depth = 0; depth < directoryParts.length; depth += 1) {
        const relativePath = directoryParts.slice(0, depth + 1).join('/');
        const entry = subfolders.get(relativePath) || { name: directoryParts[depth], path: relativePath, depth, count: 0 };
        entry.count += 1; subfolders.set(relativePath, entry);
      }
    }
    const sortedFolders = [...subfolders.values()].sort((a, b) => a.path.localeCompare(b.path, undefined, { numeric: true, sensitivity: 'base' })), rootCollapseKey = locationCollapseKey(location.id), rootCollapsed = isFolderCollapsed(rootCollapseKey);
    const folderRows = rootCollapsed ? '' : sortedFolders.filter((folder) => { const parts = folder.path.split('/'); return !parts.slice(0, -1).some((_, index) => isFolderCollapsed(locationCollapseKey(location.id, parts.slice(0, index + 1).join('/')))); }).map((folder) => { const hasChildren = sortedFolders.some((candidate) => candidate.path.startsWith(`${folder.path}/`)), collapseKey = locationCollapseKey(location.id, folder.path), collapsed = hasChildren && isFolderCollapsed(collapseKey); return `<button class="nav-item location-folder-item ${state.locationId === location.id && state.locationSubfolder === folder.path ? 'active' : ''}" style="--depth:${folder.depth}" data-location-id="${location.id}" data-subfolder="${encodeURIComponent(folder.path)}" title="${escapeHtml(`${location.path}/${folder.path}`)}"><span class="folder-tree-toggle ${hasChildren ? '' : 'empty'}" data-collapse-key="${collapseKey}" role="button" aria-label="${collapsed ? 'Expand' : 'Collapse'} ${escapeHtml(folder.name)}" aria-expanded="${!collapsed}">${hasChildren ? (collapsed ? '▸' : '▾') : ''}</span><span class="nav-icon">${iconSvg(state.library.settings?.itemIcons?.[subfolderIconKey(location.id, folder.path)] || 'folder')}</span><span class="location-name">${escapeHtml(folder.name)}</span><small>${folder.count}</small></button>`; }).join('');
...
function showAppSubmenu(group, anchor) {
  const groups = {
    ...
    help: [['Diagnostics Console', 'diagnostics', 'Ctrl+Shift+J'], ['About Pigeon', 'about', '']]
  };
...
const smartFieldOptions = [['name','Name'],['tags','Tags'],['type','Type'],['folder','Folder'],['rating','Rating'],['favorite','Favourite']];
const smartOperatorOptions = [['contains','contains'],['excludes','excludes'],['begins','begins with'],['ends','ends with'],['equals','equals'],['null','is empty'],['not-null','is not empty'],['regex','regular expression']];
...
function renderSmartFolderRules() {
  $('#smart-folder-rules').innerHTML = smartFolderRules.map((rule, index) => `<div class="smart-rule" data-rule-index="${index}"><select data-rule-part="field">${smartFieldOptions.map(([value,label]) => `<option value="${value}" ${rule.field === value ? 'selected' : ''}>${label}</option>`).join('')}</select><select data-rule-part="operator">${smartOperatorOptions.map(([value,label]) => `<option value="${value}" ${rule.operator === value ? 'selected' : ''}>${label}</option>`).join('')}</select><input data-rule-part="value" value="${escapeHtml(rule.value || '')}" placeholder="Value" ${rule.field === 'tags' ? 'data-smart-tag-input="true" aria-autocomplete="list" aria-controls="tag-autocomplete"' : ''} ${(rule.operator === 'null' || rule.operator === 'not-null') ? 'disabled' : ''}/><button data-remove-rule title="Remove rule">−</button></div>`).join(''); $('#smart-folder-rules').querySelectorAll('[data-smart-tag-input]').forEach((input) => enableTagAutocomplete(input)); updateSmartFolderFound();
}
...
$('#smart-folder-rules').addEventListener('change', (event) => { if (event.target.dataset.rulePart === 'operator' || event.target.dataset.rulePart === 'field') renderSmartFolderRules(); });
...
let diagnosticsEntries = [], diagnosticsLevel = 'all';
