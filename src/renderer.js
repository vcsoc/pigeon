...
async function openAnnotationEditor(id=state.selectedId) {
  ...
  $('#edit-brightness').value=100;$('#edit-contrast').value=100;for(const controlId of ['edit-grayscale','edit-negative','edit-sepia'])$(`#${controlId}`).setAttribute('aria-pressed','false');elements.annotationStage.style.transform='';elements.annotationStage.style.filter='';elements.annotationStage.style.backgroundImage=`url("${protectedUrl(asset.mediaUrl||asset.previewUrl)}")`;$('#annotation-filename').textContent=asset.filename||asset.name;resetAiRemovalEditor();setAnnotationTool('select');
...
$$('[data-tool]').forEach((button)=>button.addEventListener('click',()=>button.dataset.tool==='rect'?addRectangleAnnotation():setAnnotationTool(button.dataset.tool)));
function paintAiRemovalMask(event){const canvas=$('#ai-removal-mask'),x=Math.max(0,Math.min(canvas.width,event.offsetX/Math.max(1,canvas.clientWidth)*canvas.width)),y=Math.max(0,Math.min(canvas.height,event.offsetY/Math.max(1,canvas.clientHeight)*canvas.height)),context=canvas.getContext('2d');context.strokeStyle='#ff3b30';context.lineWidth=Math.max(12,Math.min(canvas.width,canvas.height)*.045);context.lineCap='round';context.lineJoin='round';if(!state.aiMaskDrawing){context.beginPath();context.arc(x,y,context.lineWidth/2,0,Math.PI*2);context.fillStyle='#ff3b30';context.fill();context.beginPath();context.moveTo(x,y);state.aiMaskDrawing=true;}else{context.lineTo(x,y);context.stroke();}state.aiMaskPainted=true;}
$('#ai-removal-mask').addEventListener('pointerdown',(event)=>{event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);state.aiMaskDrawing=false;paintAiRemovalMask(event);});$('#ai-removal-mask').addEventListener('pointermove',(event)=>{if(event.currentTarget.hasPointerCapture(event.pointerId))paintAiRemovalMask(event);});$('#ai-removal-mask').addEventListener('pointerup',(event)=>{state.aiMaskDrawing=false;event.currentTarget.releasePointerCapture(event.pointerId);});
...
$('#open-ai-removal-plugin-folder').addEventListener('click',()=>window.pigeon.openPluginsFolder());
