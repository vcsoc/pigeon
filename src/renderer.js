...
function renderPluginManager(){const query=$('#plugin-manager-search').value.trim().toLowerCase(),visible=pluginManagerPlugins.filter((plugin)=>(pluginManagerFilter==='all'||pluginManagerFilter==='installed'&&plugin.installed||pluginManagerFilter==='enabled'&&plugin.enabled)&&(!query||`${plugin.name} ${plugin.description} ${plugin.category}`.toLowerCase().includes(query)));if(!visible.some((plugin)=>plugin.id===pluginManagerSelectedId))pluginManagerSelectedId=visible[0]?.id||pluginManagerSelectedId;$('#plugin-manager-summary').textContent=`${visible.length} plugin${visible.length===1?'':'s'} · ${pluginManagerPlugins.filter((plugin)=>plugin.enabled).length} enabled`;$('#plugin-catalog').innerHTML=visible.length?visible.map((plugin)=>`<button class="plugin-card ${plugin.id===pluginManagerSelectedId?'active':''}" data-plugin-id="${escapeHtml(plugin.id)}"><span class="plugin-card-icon">${pluginIcon(plugin)}</span><span><strong>${escapeHtml(plugin.name)}</strong><small>${escapeHtml(plugin.category)} · ${escapeHtml(plugin.description)}</small></span><span class="plugin-card-status">${plugin.enabled?'Enabled':plugin.installed?'Installed':'Available'}</span></button>`).join(''):'<div class="plugin-detail-empty"><p>No plugins match this view.</p></div>';const plugin=selectedManagedPlugin(),detail=$('#plugin-detail');if(!plugin){detail.innerHTML='<div class="plugin-detail-empty"><strong>No plugin selected</strong></div>';return;}const config=(plugin.configSchema||[]).map((field)=>`<label><span>${escapeHtml(field.label)}</span><input data-plugin-config="${escapeHtml(field.key)}" type="${field.type==='number'?'number':'text'}" ${field.min!==undefined?`min="${field.min}"`:''} ${field.max!==undefined?`max="${field.max}"`:''} value="${escapeHtml(String(plugin.configured?.[field.key]??field.default??''))}" /></label>`).join('');detail.innerHTML=`<div class="plugin-detail-heading"><span class="plugin-card-icon">${pluginIcon(plugin)}</span><div><h2>${escapeHtml(plugin.name)}</h2><small>${escapeHtml(plugin.author)} · Version ${escapeHtml(plugin.version)}</small></div></div><p>${escapeHtml(plugin.description)}</p><div class="plugin-runtime-status" data-status="${escapeHtml(plugin.status)}"><strong>${escapeHtml(plugin.status)}</strong>${plugin.statusDetail?` · ${escapeHtml(plugin.statusDetail)}`:''}</div>${plugin.model?`<section class="plugin-model-card"><div><span class="plugin-card-icon">◈</span><div><strong>${escapeHtml(plugin.model.name)}</strong><small>${escapeHtml(plugin.model.type)} · ${escapeHtml(plugin.model.size)}</small></div></div><p>${escapeHtml(plugin.model.purpose)}</p><span class="plugin-model-state ${plugin.modelReady?'ready':''}">${plugin.modelReady?'Downloaded and ready':'Model not downloaded'}</span></section>`:''}<div class="plugin-detail-actions">${!plugin.installed?'<button class="primary-button" data-plugin-action="install">Download & install</button>':`<button data-plugin-action="toggle">${plugin.enabled?'Disable':'Enable'}</button>${plugin.kind==='service'?(plugin.status==='installing'?'<button disabled>Setup in progress…</button>':'<button data-plugin-action="setup">Set up dependencies & model</button>'):''}${plugin.kind==='script'?'<button data-plugin-action="run">Run now</button>':''}<button data-plugin-action="uninstall">Uninstall</button>`}</div>${plugin.installed&&config?`<section class="plugin-config"><h3>Configuration</h3>${config}<button class="primary-button" data-plugin-action="save-config">Save configuration</button></section>`:''}`;}
...
function setAnnotationTool(tool){state.annotationTool=tool;paintAnnotationToolState(tool);if(tool==='select')state.selectedAnnotationId=state.selectedAnnotationId||state.workingAnnotations.at(-1)?.id||null;renderAnnotations();}
...
$('#close-plugin-manager').addEventListener('click',()=>{stopPluginProgressUpdates();$('#plugin-manager-dialog').close();});$('#open-plugin-folder').addEventListener('click',()=>window.pigeon.openPluginsFolder());$('#plugin-manager-search').addEventListener('input',renderPluginManager);$$('[data-plugin-filter]').forEach((button)=>button.addEventListener('click',()=>{pluginManagerFilter=button.dataset.pluginFilter;$$('[data-plugin-filter]').forEach((item)=>item.classList.toggle('active',item===button));renderPluginManager();}));$('#plugin-catalog').addEventListener('click',(event)=>{const card=event.target.closest('[data-plugin-id]');if(card){pluginManagerSelectedId=card.dataset.pluginId;renderPluginManager();}});$('#plugin-detail').addEventListener('click',async(event)=>{const button=event.target.closest('[data-plugin-action]'),plugin=selectedManagedPlugin();if(!button||!plugin)return;const action=button.dataset.pluginAction;button.disabled=true;try{if(action==='install')pluginManagerPlugins=await window.pigeon.installPlugin(plugin.id);if(action==='setup'){button.textContent='Preparing setup…';startPluginProgressUpdates();try{pluginManagerPlugins=await window.pigeon.setupPlugin(plugin.id);showToast(`${plugin.name} model downloaded and ready`);}finally{stopPluginProgressUpdates();}}if(action==='toggle')pluginManagerPlugins=await window.pigeon.setPluginEnabled(plugin.id,!plugin.enabled);if(action==='save-config'){const config=Object.fromEntries([...$('#plugin-detail').querySelectorAll('[data-plugin-config]')].map((input)=>[input.dataset.pluginConfig,input.type==='number'?Number(input.value):input.value]));pluginManagerPlugins=await window.pigeon.configurePlugin(plugin.id,config);showToast(`${plugin.name} configuration saved`);}if(action==='run'){const result=await window.pigeon.runPlugin(plugin.entry);showToast(result.error||`${plugin.name} completed`);}if(action==='uninstall'&&await requestConfirmation({title:'Uninstall Plugin',message:`Uninstall “${plugin.name}” and remove its managed plugin files?`,confirmText:'Uninstall'}))pluginManagerPlugins=await window.pigeon.uninstallPlugin(plugin.id);renderPluginManager();}catch(error){showToast(error.message);await refreshPluginManager();}finally{button.disabled=false;}});
