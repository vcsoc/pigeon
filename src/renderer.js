...
function renderGrid() {
  ...
  $$('.asset-card').forEach((card) => {
    ...
    const magnifier=card.querySelector('.thumbnail-fit-preview'),hoverPreview=$('#hover-fit-preview');if(magnifier){const show=()=>{const asset=state.library.assets[rendererAssetIndexes.get(card.dataset.assetId)];if(!asset)return;hoverPreview.querySelector('img').src=protectedUrl(asset.sourceMissing?asset.previewUrl:asset.mediaUrl);hoverPreview.classList.remove('hidden');};const hide=()=>hoverPreview.classList.add('hidden');magnifier.addEventListener('pointerenter',show);magnifier.addEventListener('focus',show);magnifier.addEventListener('pointerleave',hide);magnifier.addEventListener('blur',hide);magnifier.addEventListener('click',(event)=>{event.preventDefault();event.stopPropagation();});}
