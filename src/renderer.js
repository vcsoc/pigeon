      ...
      button.addEventListener('drop',async(event)=>{
        ...
        try{
          ...
          if(movesLibraryAssets){
            const viewport=captureGridViewport(),ids=existingIds,leavesCurrent=state.locationId&&(state.locationId!==row.dataset.locationId||state.locationSubfolder!==subfolder),successorId=leavesCurrent?successorAfterRemoving(ids):null,result=await window.pigeon.moveAssetsToFolder(ids,row.dataset.locationId,subfolder),updates=new Map((result.assets||[]).map((asset)=>[asset.id,asset]));for(const[id,update]of updates){const asset=assetById(id);if(asset)Object.assign(asset,update);}for(const update of result.locations||[]){const location=state.library.locations.find((item)=>item.id===update.id);if(location)location.assetCount=update.assetCount;}updateLocationProgressUI();scheduleFolderTreeBuild();invalidateLibraryAggregates();reconcileThumbnailCards(ids,{viewport});if(leavesCurrent)selectSuccessorWithoutScrolling(successorId);if(state.library.assets.length<COOPERATIVE_VIEW_THRESHOLD)restoreGridViewport(viewport);
...
async function addAssetsToCollectionWithoutGridRefresh(ids,collection,sourceCollectionId=null){
  const viewport=captureGridViewport(),unique=[...new Set(ids)],changed=[],leavesCurrent=Boolean(sourceCollectionId&&sourceCollectionId===state.collectionId&&sourceCollectionId!==collection.id),successorId=leavesCurrent?successorAfterRemoving(unique):null,operation={collectionId:collection.id,...(sourceCollectionId&&sourceCollectionId!==collection.id?{removeCollectionId:sourceCollectionId}:{})},result=await window.pigeon.batchUpdateAssets(unique,operation,{silent:true,returnAssets:true}),updates=new Map((result.assets||[]).map((asset)=>[asset.id,asset]));
  for(const [id,update] of updates){const asset=assetById(id);if(asset){Object.assign(asset,update);changed.push(id);}}invalidateLibraryAggregates();
  reconcileThumbnailCards(changed,{viewport});if(leavesCurrent)selectSuccessorWithoutScrolling(successorId);if(state.library.assets.length<COOPERATIVE_VIEW_THRESHOLD)restoreGridViewport(viewport);showToast(`${unique.length} item${unique.length===1?'':'s'} added to ${collection.name}`);
}
