...
function attachHoverMediaPreview(card, asset, initialEvent=null) {
  ...
  const scheduleStart=(event)=>{clearTimeout(startTimer);const configuredDelay=Number(preferences.hoverPreviewDelay),delay=Math.max(0,Math.min(3000,Number.isFinite(configuredDelay)?configuredDelay:250)),modifiers={ctrlKey:Boolean(event?.ctrlKey),altKey:Boolean(event?.altKey)};if(!delay){start(modifiers);return;}startTimer=setTimeout(()=>{startTimer=null;if(hovering)start(modifiers);},delay);};
  const revealDown=(event)=>{if(protectedMotion&&hovering&&revealShortcutPressed(event)){clearTimeout(startTimer);startTimer=null;start(event,true);}},revealUp=(event)=>{if(protectedMotion&&revealShortcutPressed(event))stop();},enter=(event)=>{hovering=true;if(protectedMotion){window.addEventListener('keydown',revealDown,true);window.addEventListener('keyup',revealUp,true);}scheduleStart(event);},leave=()=>{hovering=false;window.removeEventListener('keydown',revealDown,true);window.removeEventListener('keyup',revealUp,true);stop();};
  preview.addEventListener('pointerenter', enter); preview.addEventListener('pointermove', seekFromPointer); preview.addEventListener('pointerleave', leave); card.addEventListener('dragstart', leave);card._hoverMediaWired=true;if(initialEvent)enter(initialEvent);
}
