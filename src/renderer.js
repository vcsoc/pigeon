...
function renderGrid({preserveCards=false}={}) {
  ...
  const virtualLayout=assetView.virtual?window.PigeonVirtualWindow.layout({total:allAssetCount,start:assetView.start,count:assets.length,columns:assetView.metrics.columns,rowHeight:assetView.metrics.estimatedRowHeight}):null,extentIdentity=assetView.virtual?gridViewIdentity():'';
  ...
  elements.grid.innerHTML=assetView.virtual?`<div class="virtual-card-window" style="transform:translateY(${virtualLayout.topPx}px)">${cardsHtml}</div>`:cardsHtml;
...
function reconcileActiveSmartFolderMetadataDelta(changes){
  ...
  const total=result.next.length,virtual=total>VIRTUAL_ASSET_WINDOW,metrics=virtual?virtualGridMetrics(total):null,range=window.PigeonVirtualWindow.bounded({total,start:virtual?state.virtualStart:0,size:VIRTUAL_ASSET_WINDOW,columns:metrics?.columns||1}),desiredAssets=result.next.slice(range.start,range.end).map((index)=>state.library.assets[index]).filter(Boolean),desiredIds=desiredAssets.map((asset)=>asset.id),existingById=new Map(existingCards.map((card)=>[card.dataset.assetId,card])),plan=window.PigeonMetadataViewDelta.keyedCardPlan([...existingById.keys()],desiredIds,changes.map((change)=>change.after.id));
  ...
  for(const id of plan.remove)existingById.get(id)?.remove();for(const asset of desiredAssets){let card=existingById.get(asset.id);if(!card){card=createMetadataDeltaCard(asset);existingById.set(asset.id,card);thumbnailVisibilityObserver.observe(card);}host.appendChild(card);}
  state.virtualStart=range.start;state.virtualMetrics=metrics;elements.grid.classList.toggle('virtualized-grid',virtual);elements.grid.style.height='';if(virtual){const layout=window.PigeonVirtualWindow.layout({total,start:range.start,count:desiredAssets.length,columns:metrics.columns,rowHeight:metrics.estimatedRowHeight});host.style.transform=`translateY(${layout.topPx}px)`;state.virtualExtentIdentity=metrics.identity;state.virtualExtentPx=layout.extentPx;elements.grid.style.height=`${state.virtualExtentPx}px`;}else{host.style.transform='';state.virtualExtentIdentity='';state.virtualExtentPx=0;}
...
function virtualScrollState(owner){const metrics=state.virtualMetrics,identity=gridViewIdentity(),total=metrics?.total||0,range=window.PigeonVirtualWindow.bounded({total,start:state.virtualStart,size:VIRTUAL_ASSET_WINDOW,columns:metrics?.columns||1}),logicalExtent=metrics?Math.ceil(total/metrics.columns)*metrics.estimatedRowHeight:0,cards=[...elements.grid.querySelectorAll('.asset-card')],decoded=cards.filter((card)=>card.querySelector('img.thumbnail-loaded,img[src]')).length;return{name:'virtual-scroll-state',phase:'scroll',owner,viewIdentity:identity,metricsIdentity:metrics?.identity||'',windowStart:range.start,windowEnd:range.end,resultCount:total,estimatedExtentPx:logicalExtent,fullVirtualExtentPx:state.virtualExtentPx,actualScrollHeight:elements.gridWrap.scrollHeight,scrollTop:elements.gridWrap.scrollTop,userScrollEpoch:gridScrollInteractionVersion,pendingRestore:Boolean(gridScrollRestore.pending()),domCards:cards.length,decodedCards:decoded,layout:state.layout};}
...
function updateVirtualGridWindow(){const epoch=virtualScrollEpoch;virtualScrollFrame=null;if(epoch!==gridScrollInteractionVersion)return;const metrics=state.virtualMetrics,identity=gridViewIdentity(),indices=currentViewIndices(),resultCount=indices.length;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW)return;if(metrics.identity!==identity||metrics.resultCount!==resultCount){state.virtualStart=window.PigeonVirtualWindow.bounded({total:resultCount,start:state.virtualStart,size:VIRTUAL_ASSET_WINDOW,columns:metrics.columns}).start;recordVirtualScrollState('stale-metrics',true);renderGrid({preserveCards:true});return;}const geometry=window.PigeonVirtualWindow.geometry({identity,total:resultCount,start:state.virtualStart,size:VIRTUAL_ASSET_WINDOW,columns:metrics.columns,rowHeight:metrics.estimatedRowHeight,scrollTop:elements.gridWrap.scrollTop,viewportHeight:elements.gridWrap.clientHeight}),next=window.PigeonVirtualWindow.windowForScroll({total:resultCount,currentStart:geometry.start,size:VIRTUAL_ASSET_WINDOW,columns:metrics.columns,rowHeight:metrics.estimatedRowHeight,scrollTop:geometry.scrollTop}).start;if(next===state.virtualStart&&elements.grid.querySelectorAll('.asset-card').length)return;const scroll=geometry.scrollTop;state.virtualStart=next;renderGrid({preserveCards:true});if(epoch===gridScrollInteractionVersion&&Math.abs(elements.gridWrap.scrollTop-scroll)>1){suppressGridScroll=true;elements.gridWrap.scrollTop=scroll;requestAnimationFrame(()=>{if(epoch===gridScrollInteractionVersion)suppressGridScroll=false;});}recordVirtualScrollState('window-update',true);}
...
window.pigeon.onAssetsPatched(({portfolioId,patches=[]})=>{if(portfolioId!==state.library.activePortfolioId)return;let duplicateMetadata=false,selectedChanged=false,viewChanged=false;const changes=[];for(const patch of patches){const before=snapshotAssetForMetadataDelta(assetById(patch.id));if(!assetStreamState.patch(patch.id,patch))continue;const asset=assetById(patch.id);changes.push({index:rendererAssetIndexes.get(patch.id),before,after:asset,patch});patchVisibleAssetSourceState(asset);patchCardMetadata(asset,patch);if(patch.contentHash)duplicateMetadata=true;if(asset.id===state.selectedId)selectedChanged=true;if([...METADATA_VIEW_PATCH_FIELDS].some((field)=>Object.hasOwn(patch,field)))viewChanged=true;}const deltaHandled=viewChanged&&applyMetadataViewDelta(changes);if(viewChanged&&!deltaHandled){invalidateTagCache();invalidateAssetViewCache();scheduleStreamGridRender();}if(duplicateMetadata){refreshDuplicateIds();$('#duplicates-count').textContent=String(state.duplicateIds.size);}if(selectedChanged)renderInspector();});
