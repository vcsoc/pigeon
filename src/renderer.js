...
function queueThumbnailCardsForViewport(){const viewport=elements.gridWrap.getBoundingClientRect();for(const[card,load]of activeThumbnailLoads)if(!card.isConnected||!thumbnailWithinLoadRange(card.getBoundingClientRect(),viewport))load.cancel();for(const card of elements.grid.querySelectorAll('.asset-card')){const preview=card.querySelector('.asset-preview');if(!preview?.dataset.thumbnailSrc||preview.querySelector(':scope > img.thumbnail-loaded'))continue;if(thumbnailWithinLoadRange(card.getBoundingClientRect(),viewport))pendingThumbnailCards.add(card);}scheduleSettledThumbnailLoads();}
...
function currentAssetViewSnapshot(){if(state.library.assets.length<COOPERATIVE_VIEW_THRESHOLD){const assets=filteredAssets();return{assets,total:assets.length,ready:true,virtual:false,start:0,metrics:null};}const signature=assetViewSignature(),view=cooperativeAssetView?.signature===signature?cooperativeAssetView:startCooperativeAssetView(signature),indices=view.ready?view.indices:[],total=view.ready?view.total:view.total,virtual=view.ready&&total>VIRTUAL_ASSET_WINDOW,metrics=virtual?virtualGridMetrics(total):null;let start=virtual?Math.max(0,Math.min(total-VIRTUAL_ASSET_WINDOW,state.virtualStart||0)):0;if(metrics&&start<total-VIRTUAL_ASSET_WINDOW)start=Math.floor(start/metrics.columns)*metrics.columns;return{assets:indices.slice(start,start+VIRTUAL_ASSET_WINDOW).map((index)=>state.library.assets[index]).filter(Boolean),indices,total,ready:view.ready,virtual,start,metrics};}
...
function renderSidebar(rebuildFolderTree = false) {
  ...
  const smartRows = [], appendSmartFolders = (parentId = null, depth = 0) => {
    const siblings=sortedChildren(smartFolderChildren,parentId,'smartFolders');for (const [siblingIndex,folder] of siblings.entries()) {
      const count = metrics?.smartFolderCounts.get(folder.id)||0, hasChildren = smartFolderChildren.has(folder.id), collapseKey = smartFolderCollapseKey(folder.id), collapsed = hasChildren && collapsedKeys.has(collapseKey);
...
function renderGrid({preserveCards=false}={}) {
  ...
  renderBatchBar();
  requestAnimationFrame(() => { if(!assetView.virtual&&Date.now()>=state.postMoveRevealUntil&&Math.abs(elements.gridWrap.scrollTop-state.gridScrollTop)>2&&state.virtualStart===0)elements.gridWrap.scrollTop = state.gridScrollTop; });
...
const embeddedMetadataRequests=new Set(),embeddedMetadataCache=new Map();
...
function invalidateTagCache(schedule=true){renderedTagBrowserCatalog=null;if(schedule)invalidateLibraryAggregates();else{libraryAggregateRevision+=1;libraryAggregateBuild=null;}}
...
function captureGridViewport(){return{scrollTop:Math.max(0,Number(elements.gridWrap.scrollTop)||0),virtualStart:state.virtualStart};}
function restoreGridViewport(viewport){if(!viewport)return;const restore=()=>{suppressGridScroll=true;state.virtualStart=viewport.virtualStart;state.gridScrollTop=viewport.scrollTop;elements.gridWrap.scrollTop=viewport.scrollTop;requestAnimationFrame(()=>{elements.gridWrap.scrollTop=viewport.scrollTop;requestAnimationFrame(()=>{elements.gridWrap.scrollTop=viewport.scrollTop;suppressGridScroll=false;saveNavigationState();});});};restore();}
...
let smartFolderCountGeneration=0,smartFolderCountTimer=null;
function updateSmartFolderFound(){const generation=++smartFolderCountGeneration,filters=currentSmartFolderFilters(),output=$('#smart-folder-found');clearTimeout(smartFolderCountTimer);output.textContent='Calculating…';smartFolderCountTimer=setTimeout(()=>{let index=0,count=0;const step=()=>{if(generation!==smartFolderCountGeneration||!$('#smart-folder-dialog').open)return;const started=performance.now();do{const asset=state.library.assets[index++];if(!asset.deletedAt&&!asset.locked&&matchesSavedFilters(asset,filters))count+=1;}while(index<state.library.assets.length&&performance.now()-started<5);if(index<state.library.assets.length){scheduleAssetViewTask(step,true);return;}output.textContent=`Found ${count} item${count===1?'':'s'}`;};scheduleAssetViewTask(step,true);},80);}
...
function updateVirtualGridWindow(){virtualScrollFrame=null;const metrics=state.virtualMetrics;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW)return;const firstRow=Math.max(0,Math.floor(elements.gridWrap.scrollTop/metrics.estimatedRowHeight)),bufferRows=Math.max(8,Math.floor(VIRTUAL_ASSET_WINDOW/metrics.columns/3)),target=Math.max(0,firstRow-bufferRows)*metrics.columns,maxStart=Math.max(0,metrics.total-VIRTUAL_ASSET_WINDOW),next=Math.min(maxStart,target),threshold=metrics.columns*Math.max(4,Math.floor(bufferRows/2));if(Math.abs(next-state.virtualStart)<threshold)return;const scroll=elements.gridWrap.scrollTop;state.virtualStart=next;renderGrid({preserveCards:true});elements.gridWrap.scrollTop=scroll;}
elements.gridWrap.addEventListener('scroll',()=>{const scrollTop=elements.gridWrap.scrollTop;if(Math.abs(scrollTop-thumbnailLastScrollTop)>1)thumbnailScrollDirection=scrollTop>thumbnailLastScrollTop?1:-1;thumbnailLastScrollTop=scrollTop;thumbnailScrollUntil=Date.now()+320;clearTimeout(thumbnailSettleTimer);scheduleThumbnailViewportSweep();const metrics=state.virtualMetrics;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW||virtualScrollFrame)return;virtualScrollFrame=requestAnimationFrame(updateVirtualGridWindow);},{passive:true});
...
window.pigeon.onSidebarChanged(({collections,smartFolders,locations,portfolios,settings,activePortfolioId})=>{if(activePortfolioId!==state.library.activePortfolioId)return;state.library.collections=collections||state.library.collections;state.library.smartFolders=smartFolders||state.library.smartFolders;state.library.locations=locations||state.library.locations;state.library.portfolios=portfolios||state.library.portfolios;state.library.settings={...(state.library.settings||{}),...(settings||{})};invalidateTagCache();renderSidebar(false);renderTagSuggestions();});
window.pigeon.onLibraryAssets(({ generation, assets, done }) => {
  if (generation !== state.streamGeneration) return;
  for(const asset of assets){if(asset.locked)continue;rendererAssetIndexes.set(asset.id,state.library.assets.length);state.library.assets.push(asset);}if(assets.length)invalidateTagCache(false);
  if (!done) return;
...
window.pigeon.onAssetsPatched(({portfolioId,patches=[]})=>{if(portfolioId!==state.library.activePortfolioId)return;let duplicateMetadata=false,selectedChanged=false;for(const patch of patches){const asset=state.library.assets[rendererAssetIndexes.get(patch.id)];if(!asset)continue;Object.assign(asset,patch);patchVisibleAssetSourceState(asset);if(patch.contentHash)duplicateMetadata=true;if(asset.id===state.selectedId)selectedChanged=true;}if(duplicateMetadata){refreshDuplicateIds();$('#duplicates-count').textContent=String(state.duplicateIds.size);}if(selectedChanged)renderInspector();});
window.pigeon.onThumbnailReady(({ id, previewUrl, mediaUrl, proxyPath, proxyVersion, width, height, duration, failed, error, dominantColor, histogram, palette, perceptualHash, exif, embeddedMetadata, technicalMetadata }) => {
  ...
  if ((placeholder||documentFallback) && failed) (placeholder||documentFallback).outerHTML = `<div class="asset-file asset-preview-failed" title="${escapeHtml(error || 'Preview unavailable')}"><span class="file-glyph">${iconFor(asset?.kind)}</span><span class="file-ext">${escapeHtml(asset?.extension || 'FILE')}</span><small>Preview unavailable</small></div>`;
    ...
    const image = document.createElement('img'); image.alt = asset?.name || 'Asset thumbnail'; image.loading = 'lazy'; image.addEventListener('load', () => { fitRotatedThumbnail(image);renderPixelatedCard(card);scheduleMasonry(); }, { once: true }); image.src = previewUrl; (placeholder||documentFallback).replaceWith(image);
