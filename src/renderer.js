...
function openSettings() {
  ...
  $('#thumbnail-effect-reveal-shortcut').value=preferences.thumbnailEffectRevealKey==='Control'?'Ctrl':preferences.thumbnailEffectRevealKey||'Ctrl';
...
function revealShortcutPressed(event){const configured=String(preferences.thumbnailEffectRevealKey||'Ctrl').replace('Control','Ctrl').split('+'),key=configured.at(-1),eventKey=event.key==='Control'?'Ctrl':event.key==='Meta'?'Meta':event.key.length===1?event.key.toUpperCase():event.key,required=new Set(configured.slice(0,-1));return eventKey===key&&(!required.has('Ctrl')||event.ctrlKey)&&(!required.has('Alt')||event.altKey)&&(!required.has('Shift')||event.shiftKey)&&(!required.has('Meta')||event.metaKey);}
...
$('#clear-thumbnail-effect-reveal-shortcut').addEventListener('click',()=>{preferences.thumbnailEffectRevealKey='Ctrl';$('#thumbnail-effect-reveal-shortcut').value='Ctrl';applyPreferences(true);});
