...
async function runShortcutAction(action){const ids=isInternalViewerOpen()?[state.viewerAssetId]:state.selectedIds.size?[...state.selectedIds]:state.selectedId?[state.selectedId]:[];if(!ids.length){showToast('Select one or more thumbnails first');return;}const viewport=captureGridViewport();let result;for(let index=0;index<(action.steps||[]).length;index++){const step=action.steps[index],last=index===action.steps.length-1;if(step.type==='moveFolder')result=await window.pigeon.moveAssetsToPath(ids,step.value);else if(step.type==='autoRename')result=await window.pigeon.autoRenameAssets(ids,step.value||'<modified-date-time:YYYYMMDD-HHmmss>');else result=await window.pigeon.batchUpdateAssets(ids,operationForShortcutStep(step),{silent:true,returnAssets:true});applyAssetMutationVisibility(ids,result,{viewport,reconcile:last||Boolean(result?.hiddenIds?.some((id)=>assetById(id))),sidebar:last});}const details=[];if(result?.renamed)details.push(`${result.renamed} safely renamed`);if(result?.skipped)details.push(`${result.skipped} skipped`);showToast(`${action.name} applied to ${result?.moved??ids.length} item${(result?.moved??ids.length)===1?'':'s'}${details.length?` · ${details.join(' · ')}`:''}`);}
...
function openAnnotationEditor(id=state.selectedId) {
  ...
  if(!isEditableVisualAsset(asset)||!asset.width||!asset.height){showToast('Select an online image or rendered visual document first');return;}
  state.selectedId=id;state.workingAnnotations=structuredClone(asset.annotations||[]);state.workingEdits={rotate:0,flip:false,grayscale:false,negative:false,sepia:false,brightness:1,contrast:1,crop:null};
  $('#edit-brightness').value=100;$('#edit-contrast').value=100;for(const controlId of ['edit-grayscale','edit-negative','edit-sepia'])$(`#${controlId}`).setAttribute('aria-pressed','false');elements.annotationStage.style.transform='';elements.annotationStage.style.filter='';elements.annotationStage.style.backgroundImage=`url("${protectedUrl(asset.mediaUrl||asset.previewUrl)}")`;elements.annotationStage.style.aspectRatio=`${asset.width} / ${asset.height}`;
  elements.annotationView.classList.remove('hidden');elements.grid.classList.add('hidden');elements.empty.classList.add('hidden');elements.tagBrowser.classList.add('hidden');elements.sentinel.classList.add('hidden');$('#duplicate-controls').classList.add('hidden');elements.count.textContent='Editing';elements.status.textContent=`Editing ${asset.filename}`;renderAnnotations();
}
...
function showAssetContextMenu(event, id) {
  ...
  elements.contextMenu.querySelectorAll('[data-context-action]').forEach((button) => button.addEventListener('click', async () => {
    ...
    if(action.startsWith('convert-')){const target=await window.pigeon.convertImage(id,action.slice(8));if(target)showToast(`Saved ${target}`);}
