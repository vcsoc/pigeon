...
elements.grid.addEventListener('drag',(event)=>{const card=event.target.closest?.('.asset-card');if(!card||card._pigeonNativeDragStarted||!card._pigeonDragIds?.length)return;const outside=event.screenX===0&&event.screenY===0||event.clientX<0||event.clientY<0||event.clientX>window.innerWidth||event.clientY>window.innerHeight;if(outside){card._pigeonNativeDragStarted=true;window.pigeon.startAssetDrag(card._pigeonDragIds);}});elements.grid.addEventListener('dragend',(event)=>{const card=event.target.closest?.('.asset-card');if(card){delete card._pigeonDragIds;delete card._pigeonNativeDragStarted;}});
