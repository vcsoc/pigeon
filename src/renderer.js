...
function reconcileThumbnailCards(changedIds=[],{sidebar=true,viewport=null}={}){
  if(state.library.assets.length>=COOPERATIVE_VIEW_THRESHOLD){invalidateAssetViewCache();for(const id of changedIds){state.selectedIds.delete(id);if(state.selectedId===id)state.selectedId=null;}const view=startCooperativeAssetView(assetViewSignature());view.viewportRestore=viewport;renderBatchBar();renderInspector();if(sidebar)renderSidebar(false);saveNavigationState();return;}
