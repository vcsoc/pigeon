...
async function clearTrash(){const count=state.library.assets.filter((asset)=>asset.deletedAt).length;if(!count){showToast('Trash is already empty');return;}const recycle=preferences.trashDeletionMode==='recycle',confirmed=await requestConfirmation({title:'Clear Trash',message:recycle?`Move ${count} trashed source file${count===1?'':'s'} to the operating-system Recycle Bin?`:`Permanently delete ${count} trashed source file${count===1?'':'s'}? This cannot be undone.`,confirmText:'Clear Trash'});if(!confirmed)return;try{const result=await window.pigeon.emptyTrash(preferences.trashDeletionMode);showToast(result.failed?`${result.deleted} deleted · ${result.failed} could not be deleted`:`${result.deleted} trash item${result.deleted===1?'':'s'} deleted`);}catch(error){showToast(error.message);}}
...
function showPrimarySidebarContextMenu(event){event.preventDefault();const x=event.clientX,y=event.clientY;closeFloatingMenus();const renderMenu=()=>{elements.contextMenu.innerHTML=`<div class="context-menu-label">Show in left panel</div>${primarySidebarItems.map(([view,label,key])=>`<button data-toggle-sidebar-item="${view}"><span>${preferences[key]?'✓':' '}</span><span>${label}</span></button>`).join('')}`;elements.contextMenu.querySelectorAll('[data-toggle-sidebar-item]').forEach((button)=>button.addEventListener('click',()=>{const item=primarySidebarItems.find(([view])=>view===button.dataset.toggleSidebarItem),key=item[2];preferences[key]=!preferences[key];localStorage.setItem('pigeon.preferences',JSON.stringify(preferences));window.pigeon.updatePreferences(preferences);applyPreferences(false);renderMenu();}));elements.contextMenu.classList.remove('hidden');positionMenu(elements.contextMenu,x,y);};renderMenu();}
...
$('#primary-library-nav').addEventListener('contextmenu',(event)=>{if(event.target.closest('[data-view="trash"]'))showTrashContextMenu(event);else showPrimarySidebarContextMenu(event);});
