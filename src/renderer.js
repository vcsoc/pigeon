...
function applyStaticIcons() {
  ...
  const preferenceIcons = { general:'settings', indexing:'folder', sidebar:'folder', controls:'filter', preview:'eye', screenshot:'camera', shortcuts:'key', notifications:'bell', password:'lock', 'auto-import':'download', 'ai-search':'search', 'ai-models':'smart', mcp:'link', developer:'code' };
...
let masonryFrame,navigationPaintFrame=null,navigationRenderGeneration=0,searchRenderFrame=null,marqueeSelection=null,marqueeScrollFrame=null,suppressMarqueeClick=false,hoverFitPreviewTimer=null,thumbnailObservationGeneration=0,activeMagnifierCard=null;
const pendingThumbnailCards=new Set();let thumbnailSettleTimer=null,thumbnailLoadsActive=0,thumbnailScrollUntil=0,thumbnailLastScrollTop=0,thumbnailScrollDirection=1;const MAX_THUMBNAIL_LOADS=4,THUMBNAIL_READ_AHEAD_PX=1100;
...
let hoveredVideoSoundSync=null;const activeHoverPreviewStops=new Set();function stopAllHoverPreviews(){for(const stop of [...activeHoverPreviewStops])stop();}elements.gridWrap.addEventListener('scroll',stopAllHoverPreviews,{passive:true});window.pigeon.onHoverControl?.((pressed)=>hoveredVideoSoundSync?.(pressed));
function attachHoverMediaPreview(card, asset, initialEvent=null) {
  ...
  const start=(event,revealEvent=false)=>{if(!pointerIsOverPreview()||!enabled()||media||(protectedMotion&&!state.thumbnailEffectRevealPressed&&!revealEvent))return;controlHeld=Boolean(event?.ctrlKey||event?.altKey);if(animatedImage){media=document.createElement('img');media.className='thumbnail-hover-media animated-image';media.alt='';media.src=protectedUrl(asset.mediaUrl);preview.appendChild(media);preview.classList.add('animated-hovering');return;}media=document.createElement(asset.kind);const activeMedia=media;activeMedia.className=`thumbnail-hover-media ${asset.kind}`;activeMedia.src=asset.mediaUrl;activeMedia.preload='metadata';activeMedia.loop=true;activeMedia.muted=asset.kind==='video';activeMedia.playsInline=true;preview.appendChild(activeMedia);if(asset.kind==='video'){syncHoverVideoSound();hoveredVideoSoundSync=syncHoverVideoSound;window.pigeon.monitorHoverControl(true);window.addEventListener('keydown',keyDown,true);window.addEventListener('keyup',keyUp,true);}preview.classList.add('media-hovering');activeMedia.addEventListener('loadedmetadata',()=>{if(media!==activeMedia||!pointerIsOverPreview())return;if(desiredTime!==null)activeMedia.currentTime=desiredTime;activeMedia.play().catch(()=>{});},{once:true});if(pointerIsOverPreview())activeMedia.play().catch(()=>{});};
...
function renderGrid() {
  ...
  elements.grid.innerHTML = assets.map((asset) => {
    ...
    const fullImagePreview=visual&&(asset.kind==='image'||isImagePreviewDocument(asset));
...
elements.grid.addEventListener('dragstart',(event)=>{const card=event.target.closest('.asset-card');if(!card)return;if(marqueeSelection){event.preventDefault();return;}const ids=state.selectedIds.has(card.dataset.assetId)?[...state.selectedIds]:[card.dataset.assetId];if(!event.shiftKey){event.preventDefault();window.pigeon.startAssetDrag(ids);return;}event.dataTransfer.setData('application/x-pigeon-assets',JSON.stringify(ids));event.dataTransfer.setData('application/x-pigeon-origin',JSON.stringify({collectionId:state.collectionId||null,locationId:state.locationId||null,subfolder:state.locationSubfolder||''}));const draggedAssets=ids.map((id)=>{const index=rendererAssetIndexes.get(id);return index===undefined?null:state.library.assets[index];}).filter(Boolean),paths=draggedAssets.map((asset)=>asset.path).filter(Boolean);event.dataTransfer.effectAllowed='copyMove';event.dataTransfer.setData('text/plain',paths.join('\n'));event.dataTransfer.setData('text/uri-list',paths.map((filePath)=>`file:///${filePath.replace(/\\/g,'/').replace(/^\/+/, '')}`).join('\r\n'));if(draggedAssets[0]?.mediaUrl)event.dataTransfer.setData('DownloadURL',`application/octet-stream:${draggedAssets[0].filename}:${draggedAssets[0].mediaUrl}`);});
...
const IMAGE_PREVIEW_DOCUMENT_EXTENSIONS=new Set(['AF','AFDESIGN','AFPHOTO','SNAGX']);
function isImagePreviewDocument(asset){return asset?.kind==='document'&&IMAGE_PREVIEW_DOCUMENT_EXTENSIONS.has(String(asset.extension).toUpperCase())&&Boolean(asset.proxyPath);}
...
function renderInspector() {
  ...
  const image = asset.kind === 'image' || isImagePreviewDocument(asset), video = asset.kind === 'video', audio = asset.kind === 'audio';
...
function renderInternalViewer() {
  const asset = state.library.assets.find((item) => item.id === state.viewerAssetId);
  if (!asset) return;
  const image = asset.kind === 'image' || isImagePreviewDocument(asset), video = asset.kind === 'video', audio = asset.kind === 'audio',text=['TXT','JSON','YAML','YML'].includes(String(asset.extension).toUpperCase());
...
function renderDeveloperOverlay(snapshot){const collective=snapshot.collective||{},rendererQueued=pendingThumbnailCards.size+thumbnailLoadsActive+[...backgroundTasks.values()].reduce((sum,task)=>sum+Math.max(0,(Number(task.total)||0)-(Number(task.completed)||0)),0),queued=(collective.queuedItems||0)+rendererQueued;$('#developer-overlay-metrics').innerHTML=[['CPU',`${(collective.cpu||0).toFixed(1)}%`],['GPU',`${(collective.gpuCpu||0).toFixed(1)}%`],['Memory',formatTelemetryBytes(collective.memoryBytes)],['GPU memory',formatTelemetryBytes(collective.gpuMemoryBytes)],['Queued',queued.toLocaleString()],['Workers',collective.activeThreads||0]].map(([label,value])=>`<div><small>${label}</small><strong>${value}</strong></div>`).join('');$('#developer-overlay-time').textContent=new Date(snapshot.timestamp||Date.now()).toLocaleTimeString();$('#developer-overlay-detail').textContent=`${collective.activeRuns||0} active operations · ${collective.diagnosticErrors||0} errors · ${collective.logicalCpus||0} logical CPUs · uptime ${formatMediaTime(collective.uptimeSeconds||0)}`;}
