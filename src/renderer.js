...
function renderGrid() {
  ...
  $$('.asset-card').forEach((card) => {
    ...
    const magnifier=card.querySelector('.thumbnail-fit-preview'),hoverPreview=$('#hover-fit-preview');if(magnifier){let hovering=false;const show=()=>{hovering=true;clearTimeout(hoverFitPreviewTimer);const asset=state.library.assets[rendererAssetIndexes.get(card.dataset.assetId)],image=hoverPreview.querySelector('img');if(!asset)return;image.onload=()=>{if(!hovering)return;hoverFitPreviewTimer=setTimeout(()=>{if(hovering)hoverPreview.classList.remove('hidden');},420);};image.src=protectedUrl(asset.previewUrl||asset.mediaUrl);if(image.complete)image.onload();};const hide=()=>{hovering=false;clearTimeout(hoverFitPreviewTimer);hoverPreview.classList.add('hidden');hoverPreview.querySelector('img').onload=null;hoverPreview.querySelector('img').removeAttribute('src');};magnifier.addEventListener('pointerenter',show);magnifier.addEventListener('focus',show);magnifier.addEventListener('pointerleave',hide);magnifier.addEventListener('blur',hide);magnifier.addEventListener('click',(event)=>{event.preventDefault();event.stopPropagation();});}
