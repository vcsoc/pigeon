...
async function lockSidebarFolder(control){const collapseKey=control.dataset.lockCollapseKey;if(collapseKey){paintFolderCollapseImmediately(collapseKey,true);setFolderCollapsed(collapseKey,true);}try{if(control.dataset.folderLock==='collection'){const id=control.dataset.lockCollectionId;await window.pigeon.lockCollectionNow(id);if(collectionBranchIds(id).has(state.collectionId))selectView('all');}else{const locationId=control.dataset.lockLocationId,subfolder=decodeURIComponent(control.dataset.lockSubfolder||'');await window.pigeon.lockFolderNow(locationId,subfolder);if(state.locationId===locationId&&(!subfolder||state.locationSubfolder===subfolder||state.locationSubfolder.startsWith(`${subfolder}/`)))selectView('all','All');}}catch(error){showToast(error.message);}}
...
let pluginManagerPlugins=[],pluginManagerSelectedId='ai-removal',pluginManagerFilter='all';
