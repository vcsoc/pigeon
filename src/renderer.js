...
function applyLibraryAggregateMetrics(metrics){if(!metrics)return;$('#all-count').textContent=metrics.visibleCount;$('#uncategorized-count').textContent=metrics.uncategorizedCount;$('#untagged-count').textContent=metrics.untaggedCount;$('#favorites-count').textContent=metrics.favoritesCount;$('#tags-count').textContent=metrics.tagCatalog.length;$('#offline-count').textContent=metrics.offlineCount;$('#trash-count').textContent=metrics.trashCount;for(const row of $$('.collection-item[data-collection-id]')){const count=row.querySelector(':scope > small');if(count)count.textContent=metrics.collectionCounts.get(row.dataset.collectionId)||0;}for(const row of $$('.smart-folder-item[data-smart-folder-id]')){const count=row.querySelector(':scope > small');if(count)count.textContent=metrics.smartFolderCounts.get(row.dataset.smartFolderId)||0;}}
...
function renderGrid({preserveCards=false}={}) {
  ...
  if(assetView.virtual){if(state.virtualExtentIdentity!==extentIdentity){state.virtualExtentIdentity=extentIdentity;state.virtualExtentPx=0;}state.virtualExtentPx=Math.max(state.virtualExtentPx,virtualLayout.extentPx);elements.grid.style.height=`${state.virtualExtentPx}px`;}
  ...
  renderBatchBar();
  requestAnimationFrame(()=>{if(assetView.virtual&&state.virtualExtentIdentity===extentIdentity){const liveWindow=elements.grid.querySelector('.virtual-card-window'),requiredExtent=virtualLayout.topPx+(liveWindow?.getBoundingClientRect().height||0);if(requiredExtent>state.virtualExtentPx){state.virtualExtentPx=requiredExtent;elements.grid.style.height=`${state.virtualExtentPx}px`;}}gridScrollRestore.commit({ready:assetView.ready});});
...
async function updateAssetsWithoutGridRefresh(ids,patch){const unique=[...new Set(ids)],assets=[];for(const id of unique){const asset=state.library.assets[rendererAssetIndexes.get(id)];if(!asset)continue;Object.assign(asset,patch);patchCardMetadata(asset,patch);assets.push(asset);}if(['tags','favorite','deletedAt','collectionIds','stackId','sourceMissing'].some((key)=>Object.hasOwn(patch,key)))invalidateTagCache();renderInspector();const result=await window.pigeon.batchUpdateAssets(unique,patch,{silent:true,returnAssets:true}),updates=new Map((result.assets||[]).map((asset)=>[asset.id,asset]));for(const asset of assets)if(updates.has(asset.id))Object.assign(asset,updates.get(asset.id));return result;}
...
function updateVirtualGridWindow(){virtualScrollFrame=null;const metrics=state.virtualMetrics;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW)return;const firstRow=Math.max(0,Math.floor(elements.gridWrap.scrollTop/metrics.estimatedRowHeight)),bufferRows=Math.max(8,Math.floor(VIRTUAL_ASSET_WINDOW/metrics.columns/3)),target=Math.max(0,firstRow-bufferRows)*metrics.columns,next=window.PigeonVirtualWindow.bounded({total:metrics.total,start:target,size:VIRTUAL_ASSET_WINDOW,columns:metrics.columns}).start,threshold=metrics.columns*Math.max(4,Math.floor(bufferRows/2));if(Math.abs(next-state.virtualStart)<threshold)return;const scroll=elements.gridWrap.scrollTop;state.virtualStart=next;renderGrid({preserveCards:true});elements.gridWrap.scrollTop=scroll;}
let scrollFrameSample=null,previousScrollFrame=0;function sampleScrollFrame(timestamp){if(previousScrollFrame){const frameMs=timestamp-previousScrollFrame;if(frameMs>20)window.pigeon.recordPerformanceSpan({name:'scroll-frame',frameMs,durationMs:frameMs,phase:'scroll'});}previousScrollFrame=timestamp;if(Date.now()<thumbnailScrollUntil)scrollFrameSample=requestAnimationFrame(sampleScrollFrame);else{scrollFrameSample=null;previousScrollFrame=0;}}
elements.gridWrap.addEventListener('scroll',()=>{const scrollTop=elements.gridWrap.scrollTop;if(Math.abs(scrollTop-thumbnailLastScrollTop)>1)thumbnailScrollDirection=scrollTop>thumbnailLastScrollTop?1:-1;thumbnailLastScrollTop=scrollTop;thumbnailScrollUntil=Date.now()+320;if(scrollFrameSample===null)scrollFrameSample=requestAnimationFrame(sampleScrollFrame);clearTimeout(thumbnailSettleTimer);scheduleThumbnailViewportSweep();const metrics=state.virtualMetrics;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW||virtualScrollFrame)return;virtualScrollFrame=requestAnimationFrame(updateVirtualGridWindow);},{passive:true});
...
window.pigeon.onAssetsPatched(({portfolioId,patches=[]})=>{if(portfolioId!==state.library.activePortfolioId)return;let duplicateMetadata=false,selectedChanged=false,viewChanged=false;for(const patch of patches){if(!assetStreamState.patch(patch.id,patch))continue;const asset=assetById(patch.id);patchVisibleAssetSourceState(asset);if(patch.contentHash)duplicateMetadata=true;if(asset.id===state.selectedId)selectedChanged=true;if(['tags','rating','favorite','collectionIds','deletedAt','name','filename','modified','indexedAt','size','locationId','sourceMissing','quickChecked'].some((field)=>Object.hasOwn(patch,field)))viewChanged=true;}if(viewChanged){invalidateTagCache();invalidateAssetViewCache();scheduleStreamGridRender();}if(duplicateMetadata){refreshDuplicateIds();$('#duplicates-count').textContent=String(state.duplicateIds.size);}if(selectedChanged)renderInspector();});
