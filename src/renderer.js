...
const pendingThumbnailCards=new Set();let thumbnailSettleTimer=null,thumbnailLoadsActive=0,thumbnailScrollUntil=0,thumbnailLastScrollTop=0,thumbnailScrollDirection=1;const MAX_THUMBNAIL_LOADS=16,THUMBNAIL_READ_AHEAD_PX=3600,THUMBNAIL_READ_BEHIND_PX=900;
...
function queueThumbnailCardsForViewport(){const viewport=elements.gridWrap.getBoundingClientRect(),ahead=Math.max(THUMBNAIL_READ_AHEAD_PX,viewport.height*4),behind=Math.max(THUMBNAIL_READ_BEHIND_PX,viewport.height);for(const card of elements.grid.querySelectorAll('.asset-card')){const preview=card.querySelector('.asset-preview');if(!preview?.dataset.thumbnailSrc||preview.querySelector(':scope > img.thumbnail-loaded'))continue;const rect=card.getBoundingClientRect(),distanceBefore=viewport.top-rect.bottom,distanceAfter=rect.top-viewport.bottom,within=thumbnailScrollDirection>0?distanceAfter<=ahead&&distanceBefore<=behind:distanceBefore<=ahead&&distanceAfter<=behind;if(within)pendingThumbnailCards.add(card);}scheduleSettledThumbnailLoads();}
...
function drainThumbnailLoads(){while(thumbnailLoadsActive<MAX_THUMBNAIL_LOADS&&pendingThumbnailCards.size){const card=nextThumbnailCard();if(!card)break;pendingThumbnailCards.delete(card);const preview=card.querySelector('.asset-preview'),source=preview?.dataset.thumbnailSrc;if(!preview||!source)continue;if(preview.querySelector(':scope > img.thumbnail-loaded')){preview.removeAttribute('data-thumbnail-src');continue;}thumbnailLoadsActive+=1;preview.removeAttribute('data-thumbnail-src');const image=new Image();image.alt='';image.decoding='async';image.style.transform=preview.dataset.thumbnailTransform||'';let settled=false;const finish=()=>{if(settled)return;settled=true;thumbnailLoadsActive=Math.max(0,thumbnailLoadsActive-1);queueMicrotask(drainThumbnailLoads);};image.onload=()=>{if(image.naturalWidth&&card.isConnected){image.classList.add('thumbnail-loaded');preview.appendChild(image);card.querySelector('.asset-image-placeholder')?.remove();card.classList.remove('thumbnail-load-failed');delete preview.dataset.thumbnailAttempts;if(preview.classList.contains('quarter-turned'))fitRotatedThumbnail(image);renderPixelatedCard(card);scheduleMasonry();}finish();};image.onerror=()=>{const attempts=(Number(preview.dataset.thumbnailAttempts)||0)+1;preview.dataset.thumbnailAttempts=String(attempts);if(card.isConnected&&attempts<3){preview.dataset.thumbnailSrc=source;setTimeout(()=>{if(card.isConnected){pendingThumbnailCards.add(card);scheduleSettledThumbnailLoads();}},200*attempts);}else card.classList.add('thumbnail-load-failed');finish();};image.src=source;}}
...
function renderGrid({preserveCards=false}={}) {
  ...
  const virtualTop=assetView.virtual?Math.floor(assetView.start/assetView.metrics.columns)*assetView.metrics.estimatedRowHeight:0,remaining=assetView.virtual?Math.max(0,allAssetCount-assetView.start-assets.length):0,virtualBottom=assetView.virtual?Math.ceil(remaining/assetView.metrics.columns)*assetView.metrics.estimatedRowHeight:0,viewportAssetOffset=assetView.virtual?Math.max(0,Math.floor(elements.gridWrap.scrollTop/assetView.metrics.estimatedRowHeight)*assetView.metrics.columns-assetView.start):0;
  elements.grid.innerHTML = `${virtualTop?`<div class="virtual-grid-spacer" style="height:${virtualTop}px" aria-hidden="true"></div>`:''}${assets.map((asset,assetIndex) => {
    ...
    const eagerThumbnail=visual&&assetIndex>=Math.max(0,viewportAssetOffset-(assetView.metrics?.columns||1)*2)&&assetIndex<viewportAssetOffset+64;
...
elements.gridWrap.addEventListener('scroll',()=>{const scrollTop=elements.gridWrap.scrollTop;if(Math.abs(scrollTop-thumbnailLastScrollTop)>1)thumbnailScrollDirection=scrollTop>thumbnailLastScrollTop?1:-1;thumbnailLastScrollTop=scrollTop;thumbnailScrollUntil=Date.now()+320;clearTimeout(thumbnailSettleTimer);queueThumbnailCardsForViewport();const metrics=state.virtualMetrics;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW||virtualScrollFrame)return;virtualScrollFrame=requestAnimationFrame(updateVirtualGridWindow);},{passive:true});
