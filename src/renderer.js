...
function attachHoverMediaPreview(card, asset) {
  ...
  const start=()=>{if(!enabled()||media)return;if(animatedImage){media=document.createElement('img');media.className='thumbnail-hover-media animated-image';media.alt='';media.src=protectedUrl(asset.mediaUrl);preview.appendChild(media);preview.classList.add('animated-hovering');return;}media=document.createElement(asset.kind);media.className=`thumbnail-hover-media ${asset.kind}`;media.src=asset.mediaUrl;media.preload='metadata';media.loop=true;media.muted=asset.kind==='video';media.playsInline=true;preview.appendChild(media);preview.classList.add('media-hovering');media.addEventListener('loadedmetadata',()=>{if(desiredTime!==null)media.currentTime=desiredTime;media.play().catch(()=>{});},{once:true});media.play().catch(()=>{});};
  const stop=()=>{if(!media)return;if(!animatedImage){media.pause();media.removeAttribute('src');media.load();}media.remove();media=null;desiredTime=null;preview.classList.remove('media-hovering','animated-hovering');preview.style.removeProperty('--scrub-position');};
...
function renderSidebar(rebuildFolderTree = false) {
  ...
  $$('.collection-item').forEach((button) => {
    ...
    button.addEventListener('dragleave',()=>{button.classList.remove('drag-over');clearSidebarDropMarker(button);});
...
function selectSmartFolder(id) {
  hideInternalViewer();
  state.view='all';state.smartFolderId = id; state.collectionId = null; state.locationId = null; state.locationSubfolder='';state.similarIds=null;state.selectedId = null; state.gridScrollTop = 0; clearSelection(); resetRenderLimit();
...
async function removeSelectedFromCurrentCollection() {
  ...
  const before=filteredAssets().map((asset)=>asset.id),firstIndex=Math.min(...ids.map((id)=>before.indexOf(id)).filter((index)=>index>=0));
  ...
  renderGrid();renderInspector();renderSidebar();const next=filteredAssets()[Math.min(firstIndex,Math.max(0,filteredAssets().length-1))];if(next){state.selectedId=next.id;state.selectedIds=new Set([next.id]);state.selectionAnchorId=next.id;updateCardSelectionStyles();focusSelectedAsset();}showToast(`${ids.length} item${ids.length === 1 ? '' : 's'} removed from collection`); return true;
}
...
async function addAssetsToCollectionWithoutGridRefresh(ids,collection,sourceCollectionId=null){
  ...
  const before=filteredAssets().map((asset)=>asset.id),firstIndex=Math.min(...unique.map((id)=>before.indexOf(id)).filter((index)=>index>=0));reconcileThumbnailCards(changed);if(sourceCollectionId&&sourceCollectionId===state.collectionId){const next=filteredAssets()[Math.min(firstIndex,Math.max(0,filteredAssets().length-1))];if(next){state.selectedId=next.id;state.selectedIds=new Set([next.id]);state.selectionAnchorId=next.id;updateCardSelectionStyles();focusSelectedAsset();}}showToast(`${unique.length} item${unique.length===1?'':'s'} added to ${collection.name}`);
}
...
function focusSelectedAsset(){requestAnimationFrame(()=>requestAnimationFrame(()=>{const card=elements.grid.querySelector(`[data-asset-id="${state.selectedId}"]`);if(!card)return;card.focus({preventScroll:true});card.scrollIntoView({block:'nearest',inline:'nearest',behavior:'smooth'});setTimeout(()=>{state.gridScrollTop=elements.gridWrap.scrollTop;saveNavigationState();},280);}));}
...
async function handleDeleteSelection(){const ids=state.selectedIds.size?[...state.selectedIds]:state.selectedId?[state.selectedId]:[];if(!ids.length)return;if(state.view==='trash'){await deleteTrashItems(ids);return;}await window.pigeon.batchUpdateAssets(ids,{trash:true});}
