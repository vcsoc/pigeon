...
function descendantCollapseKeys(key){const keys=[];if(key.startsWith('collection:')){const root=key.slice('collection:'.length),pending=[root];while(pending.length){const parent=pending.shift();for(const item of state.library.collections||[])if(item.parentId===parent){keys.push(collectionCollapseKey(item.id));pending.push(item.id);}}}else if(key.startsWith('smart-folder:')){const root=key.slice('smart-folder:'.length),pending=[root];while(pending.length){const parent=pending.shift();for(const item of state.library.smartFolders||[])if(item.parentId===parent){keys.push(smartFolderCollapseKey(item.id));pending.push(item.id);}}}else if(key.startsWith('location:')){const locationId=key.slice('location:'.length);for(const folder of folderTreeCache.get(locationId)?.folders||[])keys.push(locationCollapseKey(locationId,folder.path));}else if(key.startsWith('subfolder:')){const remainder=key.slice('subfolder:'.length),separator=remainder.indexOf(':'),locationId=remainder.slice(0,separator),root=remainder.slice(separator+1);for(const folder of folderTreeCache.get(locationId)?.folders||[])if(folder.path.toLowerCase().startsWith(`${root}/`))keys.push(locationCollapseKey(locationId,folder.path));}return keys;}
...
function paintFolderCollapseImmediately(key,collapsed){const controls=[...document.querySelectorAll(`[data-collapse-key="${CSS.escape(key)}"],[data-total-collapse-key="${CSS.escape(key)}"]`)],toggle=controls.find((control)=>control.hasAttribute('data-collapse-key')),row=toggle?.closest('.collection-item,.smart-folder-item,.location-folder-item,.location-root-button');if(!toggle||!row)return;for(const control of controls){control.setAttribute('aria-expanded',String(!collapsed));control.setAttribute('aria-label',`${collapsed?'Expand':'Collapse'} ${control.getAttribute('aria-label')?.replace(/^(?:Expand|Collapse)\s+/,'')||'folder'}`);control.textContent=collapsed?'＋':'−';}if(row.classList.contains('location-root-button')){const list=row.closest('.location-item')?.querySelector('.location-subfolder-list');if(list)list.hidden=collapsed;return;}const depth=Number.parseInt(row.style.getPropertyValue('--depth'),10)||0;let blockedDepth=null;for(let sibling=row.nextElementSibling;sibling;sibling=sibling.nextElementSibling){if(!sibling.matches('.collection-item,.smart-folder-item,.location-folder-item'))continue;const siblingDepth=Number.parseInt(sibling.style.getPropertyValue('--depth'),10)||0;if(siblingDepth<=depth)break;if(blockedDepth!==null&&siblingDepth<=blockedDepth)blockedDepth=null;sibling.hidden=collapsed||blockedDepth!==null;if(!sibling.hidden&&sibling.querySelector(':scope > [data-collapse-key]')?.getAttribute('aria-expanded')==='false')blockedDepth=siblingDepth;}}
function toggleFolderCollapsed(key) { const collapsed=!isFolderCollapsed(key),descendants=collapsed?descendantCollapseKeys(key):[];if(collapsed)for(const descendant of descendants)paintFolderCollapseImmediately(descendant,true);paintFolderCollapseImmediately(key,collapsed);setFolderCollapseStates(collapsed?[key,...descendants]:[key],collapsed); }
...
function renderSidebar(rebuildFolderTree = false) {
  ...
  const appendCollections = (parentId = null, depth = 0, ancestorHidden = false) => {
    const siblings=sortedChildren(collectionChildren,parentId,'collections');for (const [siblingIndex,collection] of siblings.entries()) {
      ...
      collectionRows.push(`<button class="nav-item collection-item ${siblingIndex===siblings.length-1?'tree-last':''} ${state.collectionId === collection.id ? 'active' : ''}" style="--depth:${depth};--tree-color:${treeLevelColor(depth)}" data-collection-id="${collection.id}" draggable="true" ${ancestorHidden?'hidden':''}><span class="folder-tree-toggle ${hasChildren ? '' : 'empty'}" data-collapse-key="${collapseKey}" role="button" aria-label="${collapsed ? 'Expand' : 'Collapse'} ${escapeHtml(collection.name)}" aria-expanded="${!collapsed}">${hasChildren ? (collapsed ? '＋' : '−') : ''}</span><span class="nav-icon tree-folder-icon">${iconSvg(itemIcon(collection, state.collectionId===collection.id||hasChildren&&!collapsed ? 'folder-open' : 'folder'))}${collection.locked ? `<i class="tree-lock-badge">${iconSvg('lock')}</i>` : ''}</span><span>${escapeHtml(collection.name)}</span><small>${count}</small>${hasChildren?`<span class="folder-row-collapse" data-total-collapse-key="${collapseKey}" role="button" tabindex="0" aria-label="${collapsed?'Expand':'Collapse'} ${escapeHtml(collection.name)} and subfolders" aria-expanded="${!collapsed}">${collapsed?'＋':'−'}</span>`:''}</button>`);
  ...
  const smartRows = [], appendSmartFolders = (parentId = null, depth = 0, ancestorHidden = false) => {
    const siblings=sortedChildren(smartFolderChildren,parentId,'smartFolders');for (const [siblingIndex,folder] of siblings.entries()) {
      ...
      smartRows.push(`<button class="nav-item smart-folder-item ${siblingIndex===siblings.length-1?'tree-last':''} ${state.smartFolderId === folder.id ? 'active' : ''}" style="--depth:${depth};--tree-color:${treeLevelColor(depth)}" data-smart-folder-id="${folder.id}" draggable="true" ${ancestorHidden?'hidden':''}><span class="folder-tree-toggle ${hasChildren ? '' : 'empty'}" data-collapse-key="${collapseKey}" role="button" aria-label="${collapsed ? 'Expand' : 'Collapse'} ${escapeHtml(folder.name)}" aria-expanded="${!collapsed}">${hasChildren ? (collapsed ? '＋' : '−') : ''}</span><span class="nav-icon">${iconSvg(itemIcon(folder, 'smart'))}</span><span>${escapeHtml(folder.name)}</span><small>${count===undefined?'…':count}</small>${hasChildren?`<span class="folder-row-collapse" data-total-collapse-key="${collapseKey}" role="button" tabindex="0" aria-label="${collapsed?'Expand':'Collapse'} ${escapeHtml(folder.name)} and subfolders" aria-expanded="${!collapsed}">${collapsed?'＋':'−'}</span>`:''}</button>`);
...
function setSidebarSectionExpanded(name, expanded) {
  ...
  label.setAttribute('aria-expanded', String(expanded)); content.classList.toggle('collapsed', !expanded);const control=label.querySelector('[data-section-collapse]');if(control){control.textContent=expanded?'−':'＋';control.title=`${expanded?'Collapse':'Expand'} ${label.querySelector(':scope > span')?.textContent?.trim()||'section'}`;control.setAttribute('aria-label',control.title);control.setAttribute('aria-expanded',String(expanded));}
...
$$('[data-section-collapse]').forEach((button)=>button.addEventListener('click',(event)=>{event.preventDefault();event.stopPropagation();const name=button.dataset.sectionCollapse,label=document.querySelector(`[data-section-toggle="${name}"]`);setSidebarSectionExpanded(name,label?.getAttribute('aria-expanded')!=='true');}));
