...
function renderSidebar(rebuildFolderTree = false) {
  ...
  const assets=state.library.assets,visibleAssets=[],collectionCounts=new Map(),uniqueTags=new Set();let uncategorizedCount=0,untaggedCount=0,favoritesCount=0,offlineCount=0,trashCount=0;for(const asset of assets){if(asset.deletedAt)trashCount+=1;if(isOffline(asset))offlineCount+=1;if(asset.deletedAt||asset.locked)continue;visibleAssets.push(asset);const tags=asset.tags||[],collections=asset.collectionIds||[];if(!tags.length){untaggedCount+=1;if(!collections.length)uncategorizedCount+=1;}if(asset.favorite)favoritesCount+=1;for(const tag of tags)uniqueTags.add(tag);for(const id of collections)collectionCounts.set(id,(collectionCounts.get(id)||0)+1);}
  $('#all-count').textContent=visibleAssets.length;
  $('#uncategorized-count').textContent=uncategorizedCount;
  $('#untagged-count').textContent=untaggedCount;
  $('#favorites-count').textContent=favoritesCount;
  $('#tags-count').textContent=uniqueTags.size;
  $('#offline-count').textContent=offlineCount;
  $('#duplicates-count').textContent=state.duplicateIds.size;
  $('#trash-count').textContent=trashCount;
...
window.pigeon.onLocationsChanged(({ locations, loading, totalAssets }) => {
  ...
  if(sourceStatusChanged){renderSidebar(false);for(const card of elements.grid.querySelectorAll('.asset-card'))patchVisibleAssetSourceState(state.library.assets[rendererAssetIndexes.get(card.dataset.assetId)],card);renderInspector();}
