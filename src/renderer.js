...
function renderSidebar(rebuildFolderTree = false) {
  ...
  elements.locationList.innerHTML = state.library.locations.map((location) => {
    ...
    const visibleTreeFolders=(rootLocked?[]:tree.folders.filter((folder)=>{const lock=effectiveFolderLockRule(location.id,folder.path);return!lock?.locked||lock.subfolder===String(folder.path).replace(/\\/g,'/').replace(/^\/+|\/+$/g,'').toLowerCase();}));const folderSort=sidebarSortValue('folders',`${location.id}:`);if(folderSort!=='manual')visibleTreeFolders.sort((a,b)=>{if(a.depth!==b.depth&&a.path.startsWith(`${b.path}/`))return 1;if(a.depth!==b.depth&&b.path.startsWith(`${a.path}/`))return-1;const aParent=a.path.split('/').slice(0,-1).join('/'),bParent=b.path.split('/').slice(0,-1).join('/');return aParent===bParent?compareSidebarItems(a,b,folderSort):a.path.localeCompare(b.path);});
