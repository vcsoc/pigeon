const state = {
  ...
  aiMaskPainted: false,
  workingAnnotations: [],
  workingEdits:{rotate:0,flip:false,grayscale:false,negative:false,sepia:false,brightness:1,contrast:1,crop:null},
...
$('.annotation-canvas').addEventListener('wheel',(event)=>{if(elements.annotationView.classList.contains('hidden'))return;event.preventDefault();const canvas=event.currentTarget,rect=canvas.getBoundingClientRect(),cursorX=event.clientX-rect.left,cursorY=event.clientY-rect.top,oldZoom=state.annotationZoom,factor=Math.exp(-Math.max(-120,Math.min(120,event.deltaY))*.0028),next=Math.max(.05,Math.min(12,oldZoom*factor));if(Math.abs(next-oldZoom)<.001)return;const contentX=canvas.scrollLeft+cursorX,contentY=canvas.scrollTop+cursorY;state.annotationZoom=next;applyAnnotationViewMode();requestAnimationFrame(()=>{const ratio=next/oldZoom;canvas.scrollLeft=contentX*ratio-cursorX;canvas.scrollTop=contentY*ratio-cursorY;});},{passive:false});
let annotationMiddlePan=null;$('.annotation-canvas').addEventListener('mousedown',(event)=>{if(event.button!==1)return;event.preventDefault();const canvas=event.currentTarget;annotationMiddlePan={canvas,x:event.clientX,y:event.clientY,left:canvas.scrollLeft,top:canvas.scrollTop};canvas.classList.add('middle-panning');});window.addEventListener('mousemove',(event)=>{if(!annotationMiddlePan)return;event.preventDefault();annotationMiddlePan.canvas.scrollLeft=annotationMiddlePan.left-(event.clientX-annotationMiddlePan.x);annotationMiddlePan.canvas.scrollTop=annotationMiddlePan.top-(event.clientY-annotationMiddlePan.y);},{capture:true});window.addEventListener('mouseup',(event)=>{if(event.button!==1||!annotationMiddlePan)return;annotationMiddlePan.canvas.classList.remove('middle-panning');annotationMiddlePan=null;},true);$('.annotation-canvas').addEventListener('auxclick',(event)=>{if(event.button===1)event.preventDefault();});
...
$('#annotation-layer-text').addEventListener('input',(event)=>{const item=annotationById(state.selectedAnnotationId);if(item?.type==='text'){item.text=event.target.value;item.width=Math.max(item.size*1.5,item.size*Math.max(1,item.text.length)*.68);renderAnnotations();}});
...
elements.annotationStage.addEventListener('pointermove',(event)=>updateAnnotationPointer(event));elements.annotationStage.addEventListener('mousemove',(event)=>updateAnnotationPointer(event,false));
function finishAnnotationPointer(event,requirePointerId=true){if(state.annotationDrag&&(!requirePointerId||state.annotationDrag.pointerId===event.pointerId))state.annotationDrag=null;if(annotationStart&&(!requirePointerId||annotationStart.pointerId===event.pointerId)){if(annotationStart.item&&(annotationStart.item.width<3||annotationStart.item.height<3))state.workingAnnotations=state.workingAnnotations.filter((item)=>item!==annotationStart.item);annotationStart=null;}if(event.pointerId!==undefined&&elements.annotationStage.hasPointerCapture(event.pointerId))elements.annotationStage.releasePointerCapture(event.pointerId);renderAnnotations();}
elements.annotationStage.addEventListener('pointerup',finishAnnotationPointer);elements.annotationStage.addEventListener('pointercancel',finishAnnotationPointer);window.addEventListener('mouseup',(event)=>{if(state.annotationDrag||annotationStart)finishAnnotationPointer(event,false);});
