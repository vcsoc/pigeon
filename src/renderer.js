...
function renderAnnotations() {
  const asset = assetById(state.selectedId);
  if (!asset) return;
  elements.annotationStage.querySelectorAll('.annotation-mark').forEach((mark)=>mark.remove());
  elements.annotationStage.insertAdjacentHTML('beforeend',state.workingAnnotations.map((item) => item.type === 'text'
...
async function openAnnotationEditor(id=state.selectedId) {
  ...
  state.selectedId=id;state.annotationViewMode='fit';state.annotationBaseRotation=Number(asset.rotation)||0;state.workingAnnotations=structuredClone(asset.annotations||[]);state.workingEdits={rotate:0,flip:false,grayscale:false,negative:false,sepia:false,brightness:1,contrast:1,crop:null};
  $('#edit-brightness').value=100;$('#edit-contrast').value=100;for(const controlId of ['edit-grayscale','edit-negative','edit-sepia'])$(`#${controlId}`).setAttribute('aria-pressed','false');elements.annotationStage.style.transform='';elements.annotationStage.style.filter='';elements.annotationStage.style.backgroundImage=`url("${protectedUrl(asset.mediaUrl||asset.previewUrl)}")`;$('#annotation-filename').textContent=asset.filename||asset.name;resetAiRemovalEditor();setAnnotationTool('rect');
...
$$('[data-editor-view]').forEach((button)=>button.addEventListener('click',()=>applyAnnotationViewMode(button.dataset.editorView)));
...
$$('[data-tool]').forEach((button)=>button.addEventListener('click',()=>setAnnotationTool(button.dataset.tool)));
function paintAiRemovalMask(event){const canvas=$('#ai-removal-mask'),rect=canvas.getBoundingClientRect(),x=(event.clientX-rect.left)/Math.max(1,rect.width)*canvas.width,y=(event.clientY-rect.top)/Math.max(1,rect.height)*canvas.height,context=canvas.getContext('2d');context.strokeStyle='#fff';context.lineWidth=Math.max(12,Math.min(canvas.width,canvas.height)*.045);context.lineCap='round';context.lineJoin='round';if(!state.aiMaskDrawing){context.beginPath();context.moveTo(x,y);state.aiMaskDrawing=true;}else{context.lineTo(x,y);context.stroke();}state.aiMaskPainted=true;}
