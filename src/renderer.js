...
const embeddedMetadataRequests=new Set();
function requestSelectedEmbeddedMetadata(asset){if(asset.extension!=='PNG'||asset.embeddedMetadataVersion===1||embeddedMetadataRequests.has(asset.id))return;embeddedMetadataRequests.add(asset.id);window.pigeon.getEmbeddedMetadata(asset.id).then((result)=>{asset.embeddedMetadata=result?.embeddedMetadata||null;asset.embeddedMetadataVersion=result?.embeddedMetadataVersion||1;if(state.selectedId===asset.id)renderInspector();}).catch((error)=>window.pigeon.logDiagnostic('warning','Embedded metadata lookup failed',error.message)).finally(()=>embeddedMetadataRequests.delete(asset.id));}
