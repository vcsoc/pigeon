...
const startupStartedAt=performance.now(),STARTUP_SPLASH_MINIMUM_MS=0;
...
const assetStreamState=window.PigeonAssetStreamState.create({minimumViewport:120,onReset:()=>rendererAssetDerivatives.reset(),onUpsert:(asset)=>rendererAssetDerivatives.upsert(asset),onPatch:(asset)=>rendererAssetDerivatives.upsert(asset)});
...
function relayoutVirtualGrid(anchor=captureVirtualLayoutAnchor()){const interaction=gridScrollInteractionVersion,indices=currentViewIndices(),total=indices.length;if(total<=VIRTUAL_ASSET_WINDOW){renderGrid({preserveCards:true});return;}const metrics=virtualGridMetrics(total,indices),scrollTop=anchor?window.PigeonVirtualLayout.scrollForAnchor(metrics,anchor,elements.gridWrap.clientHeight):Math.min(elements.gridWrap.scrollTop,Math.max(0,metrics.extentPx-elements.gridWrap.clientHeight)),range=window.PigeonVirtualLayout.windowForScroll({model:metrics,scrollTop,viewportHeight:elements.gridWrap.clientHeight,size:VIRTUAL_ASSET_WINDOW});state.virtualMetrics=metrics;state.virtualStart=range.start;renderGrid({preserveCards:true});if(interaction===gridScrollInteractionVersion&&Math.abs(elements.gridWrap.scrollTop-scrollTop)>1){suppressGridScroll=true;elements.gridWrap.scrollTop=scrollTop;requestAnimationFrame(()=>{if(interaction===gridScrollInteractionVersion)suppressGridScroll=false;});}}
...
function renderGrid({preserveCards=false}={}) {
  ...
  if(assets.length&&!startupSplashFinished)requestAnimationFrame(()=>{window.pigeon.recordPerformanceSpan({name:'first-grid-paint',durationMs:performance.now()-startupStartedAt,phase:'paint',size:assets.length});finishStartupSplash();});saveNavigationState();
}
...
async function renameAssetFile(assetId, requestedName) {
  ...
  try {
    ...
    const current = assetById(updated.id); if (current) Object.assign(current, updated); renderGrid(); renderInspector(); showToast(`Renamed to ${updated.filename}`); return updated;
...
function renderNavigationDestination(){
  ...
  const generation=++navigationRenderGeneration;paintActiveNavigation();updateSubfolderContentToggle();restoreScopedThumbnailSize();scheduleGridScrollRestore(state.gridScrollTop,{waitUntilReady:false});elements.gridWrap.classList.add('navigation-pending');
  if(navigationPaintFrame)cancelAnimationFrame(navigationPaintFrame);
  navigationPaintFrame=requestAnimationFrame(()=>requestAnimationFrame(()=>{if(generation!==navigationRenderGeneration)return;navigationPaintFrame=null;renderGrid();renderInspector();if(!cooperativeAssetView||cooperativeAssetView.ready)elements.gridWrap.classList.remove('navigation-pending');}));
}
...
function reconcileActiveSmartFolderMetadataDelta(changes){
  ...
  const total=result.next.length,virtual=total>VIRTUAL_ASSET_WINDOW,metrics=virtual?virtualGridMetrics(total,result.next):null,range=virtual?window.PigeonVirtualLayout.bounded({model:metrics,start:state.virtualStart,size:VIRTUAL_ASSET_WINDOW}):{start:0,end:total},desiredAssets=result.next.slice(range.start,range.end).map((index)=>state.library.assets[index]).filter(Boolean),desiredIds=desiredAssets.map((asset)=>asset.id),existingById=new Map(existingCards.map((card)=>[card.dataset.assetId,card])),plan=window.PigeonMetadataViewDelta.keyedCardPlan([...existingById.keys()],desiredIds,changes.map((change)=>change.after.id));
...
elements.assetName.addEventListener('keydown', (event) => {
  ...
  else if (event.key === 'Escape') { event.preventDefault(); const asset = assetById(state.selectedId); elements.assetName.value = asset?.name || ''; elements.assetName.blur(); }
});
...
function updateVirtualGridWindow(){const epoch=virtualScrollEpoch;virtualScrollFrame=null;if(epoch!==gridScrollInteractionVersion)return;const metrics=state.virtualMetrics,indices=currentViewIndices(),resultCount=indices.length;if(!metrics||metrics.total<=VIRTUAL_ASSET_WINDOW)return;const current=virtualGridMetrics(resultCount,indices),viewportHeight=elements.gridWrap.clientHeight;if(metrics.identity!==current.identity||metrics.resultCount!==resultCount){const anchor=window.PigeonVirtualLayout.anchorForScroll(metrics,elements.gridWrap.scrollTop),scroll=window.PigeonVirtualLayout.scrollForAnchor(current,anchor,viewportHeight),range=window.PigeonVirtualLayout.windowForScroll({model:current,scrollTop:scroll,viewportHeight,size:VIRTUAL_ASSET_WINDOW});state.virtualMetrics=current;state.virtualStart=range.start;recordVirtualScrollState('stale-metrics',true);renderGrid({preserveCards:true});if(epoch===gridScrollInteractionVersion&&Math.abs(elements.gridWrap.scrollTop-scroll)>1){suppressGridScroll=true;elements.gridWrap.scrollTop=scroll;requestAnimationFrame(()=>{if(epoch===gridScrollInteractionVersion)suppressGridScroll=false;});}return;}const scroll=Math.max(0,Math.min(Math.max(0,metrics.extentPx-viewportHeight),elements.gridWrap.scrollTop)),next=window.PigeonVirtualLayout.windowForScroll({model:metrics,scrollTop:scroll,viewportHeight,size:VIRTUAL_ASSET_WINDOW}).start;if(next===state.virtualStart&&elements.grid.querySelectorAll('.asset-card').length)return;state.virtualStart=next;renderGrid({preserveCards:true});if(epoch===gridScrollInteractionVersion&&Math.abs(elements.gridWrap.scrollTop-scroll)>1){suppressGridScroll=true;elements.gridWrap.scrollTop=scroll;requestAnimationFrame(()=>{if(epoch===gridScrollInteractionVersion)suppressGridScroll=false;});}recordVirtualScrollState('window-update',true);}
...
function scheduleStreamGridRender(firstUsable=false){if(streamRenderTimer!==null)return;streamRenderTimer=requestAnimationFrame(()=>{streamRenderTimer=null;render();if(firstUsable)streamFirstPaintGeneration=state.streamGeneration;});}
...
window.pigeon.onLibraryChanged((library) => {
  ...
  render();if(!state.library.assetStreamPending)scheduleFolderTreeBuild();
});
