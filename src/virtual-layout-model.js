(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.PigeonVirtualLayout=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function build({mode='grid',width=1,cardWidth=240,gap=4,metaHeight=28,ratios=[]}={}){
    ...
    let extentPx=0,maxItemHeight=0,occupiedArea=0;for(const item of items){extentPx=Math.max(extentPx,item.y+item.height);maxItemHeight=Math.max(maxItemHeight,item.height);occupiedArea+=item.width*item.height;}const density=extentPx?occupiedArea/(width*extentPx):1;
    return{mode,width,cardWidth,gap,metaHeight,columns,targetHeight,items,total:items.length,extentPx,maxItemHeight,density,key:[mode,Math.round(width),Math.round(cardWidth),gap,metaHeight,ratios.length,ratioHash(ratios)].join(':')};
  }
  ...
  function windowForScroll({model,scrollTop=0,viewportHeight=0,size=480,overscanPx}={}){
    if(!model?.total)return{start:0,end:0,count:0,virtual:false};scrollTop=Math.max(0,Number(scrollTop)||0);viewportHeight=Math.max(0,Number(viewportHeight)||0);overscanPx=Math.max(viewportHeight*.75,Number(overscanPx)||0);const lower=Math.max(0,scrollTop-overscanPx-model.maxItemHeight),upper=scrollTop+viewportHeight+overscanPx;let first=0,last=model.total;
    while(first<model.total&&model.items[first].y+model.items[first].height<lower)first+=1;last=first;while(last<model.total&&model.items[last].y<=upper)last+=1;
    const needed=Math.max(1,last-first),count=Math.min(Math.max(needed,Math.min(size,model.total)),size),start=Math.max(0,Math.min(model.total-count,first-Math.floor((count-needed)/2)));return{start,end:Math.min(model.total,start+count),count:Math.min(count,model.total-start),virtual:model.total>size};
  }
