...
test('the supplied transparent Pigeon artwork appears on every loading surface while footer progress stays removed',async()=>{
  const logoPath=path.join(root,'pigeon-logo.png'),metadata=await sharp(logoPath).metadata(),stats=await sharp(logoPath).stats();assert.equal(metadata.format,'png');assert.equal(metadata.hasAlpha,true);assert.equal(stats.isOpaque,false);assert.match(html,/id="startup-splash"[^\n]*src="\.\.\/pigeon-logo\.png"/);assert.match(html,/id="portfolio-switch-loading"[^>]*>[\s\S]*?src="\.\.\/pigeon-logo\.png"/);assert.match(html,/id="opening-location-animation"[^>]*src="\.\.\/pigeon-logo\.png"/);assert.doesNotMatch(html,/pigeon-loading\.gif/);assert.doesNotMatch(html,/status-progress-pigeon/);assert.match(renderer,/emptyFolderArt\.classList\.toggle\('hidden',loading\)/);assert.match(styles,/\.opening-location-animation\{[^}]*object-fit:contain/);assert.match(styles,/#grid-wrap:has\(\.opening-location-animation:not\(\.hidden\)\)\{background:#040405\}/);assert.match(styles,/\.portfolio-switch-loading\{[^}]*background:#040405/);assert.doesNotMatch(styles,/\.status-progress-pigeon\{/);assert.match(packageJson,/"pigeon-logo\.png"/);assert.doesNotMatch(packageJson,/"pigeon-loading\.gif"/);
});
