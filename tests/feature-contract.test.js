...
test('polished tree, stable post-move reveal and configurable thumbnail privacy effects are wired',()=>{
  ...
  assert.match(renderer,/postMoveRevealUntil=Date\.now\(\)\+900/);assert.match(renderer,/focusSelectedAsset\(\{lockScroll:true,block:'center'\}\)/);assert.match(renderer,/if\(selectedCardFullyVisible\(visibleCard\)\)/);assert.match(renderer,/delays=lockScroll&&block==='center'\?\[0,80,200,500,1000,1800,3000,5000\]/);assert.match(renderer,/interaction!==gridScrollInteractionVersion/);
...
test('animated image hover, viewport magnifier, reorder markers and sidebar-only mutation broadcasts are wired',()=>{
  ...
  assert.match(main,/collection:rename[\s\S]{0,220}broadcastSidebar\(\)/);assert.match(main.slice(main.indexOf("ipcMain.handle('collection:move'"),main.indexOf("ipcMain.handle('collection:set-password'")),/broadcastSidebar\(\)/);assert.match(main.slice(main.indexOf("ipcMain.handle('smart-folder:move'"),main.indexOf("ipcMain.handle('smart-folder:remove'")),/broadcastSidebar\(\)/);assert.match(main,/sidebar:reorder-items[\s\S]{0,1000}broadcastSidebar\(\)/);
});
...
test('external drops use a managed organization inbox and ordinary wheel input scrolls', () => {
  ...
  assert.match(renderer, /assets = assets\.filter\(createUncategorizedPredicate\(\)\)/);
...
test('Rows is the default layout and layout names stay concise', () => {
  ...
  assert.match(packageJson, /"version": "0\.3\.6"/);
});
...
test('explicit folder rescans scope indexing and missing-preview retries without rebuilding valid cached previews',()=>{const rescanHandler=main.slice(main.indexOf("ipcMain.handle('library:rescan'"),main.indexOf("ipcMain.handle('library:refresh-sources'")),scanBody=main.slice(main.indexOf('async function scanLocation'),main.indexOf('function resumePendingScans'));assert.match(preload,/rescan: \(id, subfolder = ''\)/);assert.match(renderer,/rescan\(location\.id,subfolder\)/);assert.match(rescanHandler,/scanLocation\(id,\{rebuildPreviews:true,subfolder\}\)/);assert.match(rescanHandler,/notify:true,rebuildPreviews:true/);assert.doesNotMatch(rescanHandler,/\bbroadcast\(\)/);assert.match(scanBody,/const scanRoot=path\.resolve\(location\.path,scanSubfolder\)/);assert.match(scanBody,/assetInScanScope=\(asset\)=>asset\.locationId===location\.id&&pathIsInside\(scanRoot,asset\.path\)/);assert.match(scanBody,/walkFolder\(scanRoot/);assert.match(scanBody,/jobLibrary\.assets\.filter\(\(asset\)=>!assetInScanScope\(asset\)\)\.concat\(retained\)/);assert.doesNotMatch(scanBody,/await rebuildThumbnails\(rebuildIds\)/);assert.match(scanBody,/previewQueue\.add\(waveAssets\)/);assert.match(scanBody,/if\(rebuildPreviews&&!permissionDenied&&!asset\.thumbnailPath\)/);assert.match(main,/scanLocation\(location\.id,\{reason:'filesystem-change',subfolder\}\)/);});
...
test('asset duplication copies its preview and incrementally inserts and resorts one card',()=>{
  ...
  assert.match(main,/async function copyDuplicatePreview/);assert.match(main,/fsp\.copyFile\(source\.thumbnailPath,thumbnailTarget\)/);assert.match(duplicateSource,/watcherIgnoreUntil\.set/);assert.match(duplicateSource,/broadcastSidebar\(\)/);assert.doesNotMatch(duplicateSource,/locations\.push|watchLocation\(/);assert.doesNotMatch(duplicateSource,/\bbroadcast\(\)/);assert.match(renderer,/function stageDuplicatedAssetCard/);assert.match(renderer,/cloneNode\(true\)/);assert.match(renderer,/async function duplicateAssetsWithoutGridRefresh/);assert.match(renderer,/reconcileThumbnailCards\(added,\{sidebar:false,viewport\}\)/);assert.match(renderer,/duplicateAssetsWithoutGridRefresh\(selectedIds\)/);
});
...
test('password protection immediately hides collection and physical-folder descendants until temporary unlock',()=>{
  ...
  assert.match(main,/settings:\{\.\.\.settings,preferences:\{[^\r\n]+folderLocks:publicFolderLocks\(\)\}/);
...
test('right-panel threads and scoped analytics are wired', () => {
  ...
  assert.match(main, /Checking for changes in/);assert.match(main, /label: indexingLabel/);
...
test('the transparent VCSOC logo remains branded while the animated pigeon stays on every loading surface',async()=>{
  const logoPath=path.join(root,'pigeon-logo.png'),logoMetadata=await sharp(logoPath).metadata(),logoStats=await sharp(logoPath).stats(),animation=fs.readFileSync(path.join(root,'pigeon-busy-transparent.webm'));assert.equal(logoMetadata.format,'png');assert.equal(logoMetadata.hasAlpha,true);assert.equal(logoStats.isOpaque,false);assert.ok(animation.includes(Buffer.from('V_VP9')));assert.ok(!animation.includes(Buffer.from('A_OPUS')));
  assert.match(html,/id="startup-splash"[^\n]*src="\.\.\/pigeon-busy-transparent\.webm"/);assert.match(html,/id="portfolio-switch-loading"[^>]*>[\s\S]*?src="\.\.\/pigeon-busy-transparent\.webm"/);assert.match(html,/id="opening-location-animation"[^>]*src="\.\.\/pigeon-busy-transparent\.webm"/);assert.equal((html.match(/data-busy-animation[^>]*autoplay loop muted playsinline/g)||[]).length,3);assert.doesNotMatch(html,/pigeon-loading\.gif|status-progress-pigeon/);assert.match(renderer,/emptyFolderArt\.classList\.toggle\('hidden',loading\)/);assert.match(styles,/\.opening-location-animation\{[^}]*object-fit:contain/);assert.match(styles,/\.startup-loader video\{[^}]*background:transparent/);assert.match(packageJson,/"pigeon-logo\.png"/);assert.match(packageJson,/"pigeon-busy-transparent\.webm"/);
});
...
test('cloud placeholders remain pending and never enter scan, hash, or thumbnail reads',()=>{
  assert.match(main,/inspectCloudFiles\(batch/);assert.match(main,/placeholder: true/);assert.match(main,/const sourcePending = Boolean\(inspection\?\.placeholder\)/);assert.match(main,/if\(!\(await pathAvailable\(fingerprintSource\.path\)\)\)\{if\(currentFingerprintAsset\(fingerprintSource\)\)markAssetSourcePending/);assert.match(main,/asset\.sourcePending\|\|asset\.sourceMissing\)return/);
});
...
test('live imports refresh clean cards, context menus clear the footer, and persistent Threads uses bottom inspector tabs',()=>{
  ...
  assert.match(renderer,/renderGrid\(\{preserveCards:!view\.forceFreshCards\}\)/);assert.match(renderer,/view\.preserveCards=true/);
