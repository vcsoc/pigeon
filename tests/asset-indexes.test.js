...
test('derived indexes update search, facets, and sort keys incrementally',()=>{
  const indexes=create(),asset={id:'a',filename:'Sunset.JPG',path:'C:/Photos/Sunset.JPG',kind:'image',locationId:'l1',tags:['Travel'],rating:4,modified:20};indexes.upsert(asset);
  assert.equal(indexes.searchMatches(asset,'c:/photos/sunset'),true);assert(indexes.snapshot().byTag.get('travel').has('a'));assert.equal(indexes.sortValue(asset,'modified'),20);
