...
test('drag capture is injected on HTTP sites and targets the Downloads collection', () => {
  ...
  assert.match(worker, /body: JSON\.stringify\(\{ url, collection \}\)/);
  assert.match(worker, /controller\.abort\(\), 10 \* 60 \* 1000/);
  assert.doesNotMatch(worker, /attempt < 6/);
  assert.match(content, /Downloading in Pigeon…/);
  assert.match(content, /response\?\.ok===false/);
  assert.match(content, /response\?\.mode==='linked'/);
...
test('browser build emits installable packages for the main browser families', () => {
  execFileSync(process.execPath, [path.join(root, 'scripts', 'build-browser-extensions.js')], { cwd: root, timeout: 10000 });
  for (const browser of ['chrome', 'edge', 'brave', 'opera', 'vivaldi', 'firefox', 'safari']) {
