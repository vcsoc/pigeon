...
test('delta asset and metadata writes preserve unrelated rows',()=>{const file=temporary('delta.db'),store=createLibraryStore(file);store.save(core.migrateLibrary({settings:{autoTag:true},locations:[{id:'l',path:'/root'}],assets:[{id:'a',rating:0},{id:'b',rating:0}],collections:[{id:'c',name:'Before'}]}));assert.equal(store.upsertAssets([{id:'a',rating:5}]),1);assert.equal(store.saveLibraryMetadata(core.migrateLibrary({settings:{autoTag:false},locations:[{id:'l',path:'/root'}],collections:[{id:'c',name:'After'}],assets:[]}))>0,true);let loaded=store.load();assert.equal(loaded.assets.find((asset)=>asset.id==='a').rating,5);assert.equal(loaded.assets.find((asset)=>asset.id==='b').rating,0);assert.equal(loaded.settings.autoTag,false);assert.equal(loaded.collections[0].name,'After');assert.equal(store.deleteAssets(['a']),1);loaded=store.load();assert.deepEqual(loaded.assets.map((asset)=>asset.id),['b']);store.close();});
